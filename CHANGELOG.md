...
## Features
- **Rebrand**: HydraROUTER v1.0.0 — sidebar/app name is now `HydraROUTER`
- **nVidia combo**: idle-probe timeout cut to 8s for fast rotation; probe success now records model liveness so follow-up requests skip re-probing
- **Error learning**: `openai_max_tokens` pattern now covers `opencode`/`opencode-go` and `max_output_tokens` minimum (`>= 16`) violations with auto-correct to the floor value

## Fixes
- **nVidia provider**: removed `minimaxai/minimax-m3` from model catalog and capabilities
- **Security**: Google OAuth client id/secrets (Antigravity, Gemini CLI) are now env-only (`HYDRAROUTER_*`, see `.env.example`) — no secret committed; baseline snapshot scrubs secrets
...
- **CLI tools**: send the API key Codex actually reads —
  `[model_providers.hydrarouter.http_headers]` instead of auth.json (which left
...
- **Providers**: add Self-hosted STT / TTS / Embedding — point hydrarouter at your own
...
- **RTK**: `X-hydrarouter-Token-Saver` header to bypass token savers per request
...
## Docs
- README: add Indonesian hydrarouter tutorial video (#1709)
