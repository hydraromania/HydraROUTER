<div align="center">
  <img src="./images/hydrarouter.png?1" alt="hydrarouter Dashboard" width="800"/>
  
  # hydrarouter - FREE AI Router & Token Saver
...
[🚀 Quick Start](#-quick-start) • [💡 Features](#-key-features) • [📖 Setup](#-setup-guide) • [🌐 Website](https://hydrarouter.com)
...
This repository package is private (`hydrarouter-app`), so source/Docker execution is the expected local development path.
...
<table>
  ...
  <tr>
    ...
    <td align="center" width="320">
      ...
      <b>🇻🇳 Tiếng Việt</b><br/>
      <sub>Hướng Dẫn Setup OpenClaw + hydrarouter: Tạo Bot Zalo AI Tự Động Từ A-Z<br/>by <a href="https://github.com/tuanminhhole">tuanminhhole</a></sub>
    </td>
...
> 🎬 **Made a video about hydrarouter?** Submit a [Pull Request](https://github.com/decolua/hydrarouter/pulls) adding your video to this section — we'll merge it!

---

## 🛠️ Supported CLI Tools

hydrarouter works seamlessly with all major AI coding tools:
...
Set `X-hydrarouter-Token-Saver: off` to bypass all token savers for one chat request.
...
Headroom is optional and runs separately. hydrarouter calls Headroom's local `/v1/compress` endpoint, then keeps normal routing, fallback, auth, and usage tracking:
...
If Headroom is down or returns an error, hydrarouter fails open and sends the original request.
...
- Your CLI tool sends OpenAI format → hydrarouter translates → Provider receives native format
...
> hydrarouter itself **never charges** you anything. You only pay providers directly (if using paid services).
>
...
### 📊 Understanding hydrarouter Costs & Billing

**hydrarouter Billing Reality:**

✅ **hydrarouter software = FREE forever** (open source, never charges)  
...
❌ **hydrarouter never sends invoices** or charges your card
...
- **Cheap providers** (GLM, MiniMax): Pay them directly, hydrarouter just routes
- **FREE providers** (iFlow, Kiro, Qwen): Genuinely free forever, no hidden charges
- **hydrarouter**: Never charges anything, ever
...
The dashboard tracks your token usage and displays **estimated costs** as if you were using paid APIs directly. This is **not actual billing** - it's a reference to show how much you're saving by using free models or existing subscriptions through hydrarouter.
...
<details>
<summary><b>💳 Will I be charged by hydrarouter?</b></summary>

**No.** hydrarouter is free, open-source software that runs on your own computer. It never charges you anything.

**You only pay:**

- ✅ **Subscription providers** (Claude Code $20/mo, Codex $20-200/mo) → Pay them directly on their websites
- ✅ **Cheap providers** (GLM, MiniMax) → Pay them directly, hydrarouter just routes your requests
- ❌ **hydrarouter itself** → **Never charges anything, ever**

hydrarouter is a local proxy/router. It doesn't have your credit card, can't send invoices, and has no billing system. It's completely free software.
...
hydrarouter just routes your requests to them - there's no "catch" or future billing from hydrarouter itself. They're truly free services, and hydrarouter makes them easy to use with fallback support.
...
3. **Use subscription providers last:**
   ...
   - hydrarouter helps maximize their value through quota tracking
...
hydrarouter's smart fallback prevents surprise charges:
...
**You're in control:** Set spending limits per provider in dashboard, and hydrarouter respects them.
...
**Pro Tip:** Use Opus for complex tasks, Sonnet for speed. hydrarouter tracks quota per model!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [from hydrarouter dashboard]
...
> **Note:** OpenClaw only works with local hydrarouter. Use `127.0.0.1` instead of `localhost` to avoid IPv6 resolution issues.
...
docker pull decolua/hydrarouter:latest   # update to latest
```

**Data persistence:** `$HOME/.hydrarouter/db/data.sqlite` on host ↔ `/app/data/db/data.sqlite` in container.

### Environment Variables

| Variable                                             | Default                                  | Description                                                                         |
| ---------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------- |
| `JWT_SECRET`                                         | Auto-generated (`~/.hydrarouter/jwt-secret`) | JWT signing secret for dashboard auth cookie (override to share across instances)   |
| `INITIAL_PASSWORD`                                   | `123456`                                 | First login password when no saved hash exists                                      |
| `DATA_DIR`                                           | `~/.hydrarouter`                             | Main app data location (SQLite at `$DATA_DIR/db/data.sqlite`)                       |
| `PORT`                                               | framework default                        | Service port (`20128` in examples)                                                  |
| `HOSTNAME`                                           | framework default                        | Bind host (Docker defaults to `0.0.0.0`)                                            |
| `NODE_ENV`                                           | runtime default                          | Set `production` for deploy                                                         |
| `BASE_URL`                                           | `http://localhost:20128`                 | Server-side internal base URL used by cloud sync jobs                               |
| `CLOUD_URL`                                          | `https://hydrarouter.com`                    | Server-side cloud sync endpoint base URL                                            |
| `NEXT_PUBLIC_BASE_URL`                               | `http://localhost:3000`                  | Backward-compatible/public base URL (prefer `BASE_URL` for server runtime)          |
| `NEXT_PUBLIC_CLOUD_URL`                              | `https://hydrarouter.com`                    | Backward-compatible/public cloud URL (prefer `CLOUD_URL` for server runtime)        |
...
- Both `${DATA_DIR}` and `~/.hydrarouter` resolve to the same location in a Docker container — the symlink `/root/.hydrarouter -> /app/data` is created at build time.
...
- **Website**: [hydrarouter.com](https://hydrarouter.com)
...
Thanks to all contributors who helped make hydrarouter better!
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — A full-featured TypeScript fork of hydrarouter. Adds 36+ providers, 4-tier auto-fallback, multi-modal APIs (images, embeddings, audio, TTS), circuit breaker, semantic cache, LLM evaluations, and a polished dashboard. 368+ unit tests. Available via npm and Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — Rust token-saver. hydrarouter ports its compression pipeline to JS → **−20-40% input tokens** on every request.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) by **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. hydrarouter adapts its prompt → **−65% output tokens**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) by **[@DietrichGebert](https://github.com/DietrichGebert)** — _"lazy senior dev"_ skill. hydrarouter injects its YAGNI-first ladder → **fewer tokens, less code, shorter diffs**.

Huge thanks to these authors — without their work, hydrarouter's token-saving features wouldn't exist. ⭐ them on GitHub!
