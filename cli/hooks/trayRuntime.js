...
function cleanupLegacySystray({ silent = false } = {}) {
  // 1) Runtime dir: ~/.HydraROUTER/runtime/node_modules/systray (or %APPDATA% on Win)
  // 2) npm global nested: <npm_prefix>/node_modules/HydraROUTER/node_modules/systray
  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent) console.log(`[hydrarouter][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent) console.warn(`[hydrarouter][runtime] failed to remove ${dir}: ${e.message}`);
      }
    }
  }
}
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[hydrarouter][runtime] chmod tray bin failed: ${e.message}`);
  }
}
