...
async function buildCodexHeader() {
  ...
  const { installed, hasHydraROUTER, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasHydraROUTER, settings } = result.data;
  ...
  // Extract HydraROUTER custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:HydraROUTER-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract HydraROUTER provider config
  const provider = settings?.models?.providers?.["HydraROUTER"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("HydraROUTER/") ? primary.replace("HydraROUTER/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasHydraROUTER, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use HydraROUTER\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
