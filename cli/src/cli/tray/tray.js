...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `hydrarouter (Port ${port})`, tooltip: "Server is running", enabled: false },
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[hydrarouter] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[hydrarouter] tray init error: ${err.message}\n`);
    return null;
  }
}
