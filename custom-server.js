...
// Per-process secret proving x-hr-real-ip was stamped below rather than sent by the client.
// A bare `next start` / `next dev` never loads this file, so it cannot produce a matching
// header even though the env var is inherited by child processes. Named like x-hr-cli-token
...
http.createServer = (...args) => {
  ...
  const wrapped = (req, res) => {
    ...
    delete req.headers["x-hr-real-ip"];
    delete req.headers["x-forwarded-for"];
    delete req.headers["x-hr-via-proxy"];
    delete req.headers["x-hr-peer-token"];
    req.headers["x-hr-real-ip"] = ip;
    req.headers["x-hr-peer-token"] = PEER_TOKEN;
    if (viaProxy) req.headers["x-hr-via-proxy"] = "1";
