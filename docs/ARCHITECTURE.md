# hydrarouter Architecture

_Last updated: 2026-02-06_

## Executive Summary

hydrarouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[hydrarouter Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.hydrarouter/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.hydrarouter/usage.json`, `~/.hydrarouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.hydrarouter/db.json`)
- usage stats: `~/.hydrarouter/usage.json`
- request log lines: `~/.hydrarouter/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[hydrarouter Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.hydrarouter` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/hydrarouter && npm run build`
- Build Docker image: `cd /root/dev/hydrarouter && docker build -t hydrarouter .`
