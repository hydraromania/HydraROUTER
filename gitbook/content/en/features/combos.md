...
Combos are **custom fallback chains** that you create in the dashboard. Instead of using a single model, you define a sequence of models that hydrarouter tries in order.
...
hydrarouter automatically tries each model in sequence until one succeeds.
...
When limit reached, hydrarouter skips paid models and uses free tier only.
