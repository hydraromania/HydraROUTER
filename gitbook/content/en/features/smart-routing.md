...
hydrarouter automatically routes your requests through the best available provider using a 3-tier fallback system. Never stop coding due to quota limits or rate limiting.

---

## How It Works

hydrarouter uses intelligent routing to maximize your existing subscriptions, minimize costs, and ensure 24/7 availability:

```
Request → hydrarouter → Check Tier 1 (Subscription)
                     ↓ quota exhausted
                     Check Tier 2 (Cheap)
                     ↓ budget limit
                     Check Tier 3 (Free)
...
hydrarouter monitors quota in real-time and switches providers automatically:
...
hydrarouter selects the best model based on:
...
When budget reached, hydrarouter automatically switches to free tier.
