...
Despliega hydrarouter en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.hydrarouter` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/hydrarouter
sudo chown $USER:$USER /var/lib/hydrarouter
```
...
```bash
# Ver logs
pm2 logs hydrarouter

# Reiniciar aplicación
pm2 restart hydrarouter

# Detener aplicación
pm2 stop hydrarouter
...
Crea `/etc/nginx/sites-available/hydrarouter`:
...
sudo ln -s /etc/nginx/sites-available/hydrarouter /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de hydrarouter
...
# Actualizar hydrarouter
cd /path/to/hydrarouter/app
git pull
npm install
npm run build
pm2 restart hydrarouter
...
tar -czf hydrarouter-backup-$(date +%Y%m%d).tar.gz /var/lib/hydrarouter

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/hydrarouter-$(date +\%Y\%m\%d).tar.gz /var/lib/hydrarouter
```
...
# Ver logs
pm2 logs hydrarouter --lines 100
...
```bash
# Verificar si hydrarouter está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/hydrarouter
chmod 755 /var/lib/hydrarouter
```
