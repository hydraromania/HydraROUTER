...
Ejecuta hydrarouter en tu máquina local para desarrollo y uso personal.

---

## 📦 Instalación

Instala hydrarouter globalmente vía npm:
...
Inicia hydrarouter con un solo comando:
...
- **Directorio de datos**: `~/.hydrarouter`
...
```bash
DATA_DIR=/path/to/data hydrarouter
```
...
Presiona `Ctrl+C` en la terminal donde hydrarouter se está ejecutando.

```bash
# En la terminal ejecutando hydrarouter
^C  # Presiona Ctrl+C
```
...
sudo npm install -g hydrarouter
...
```bash
# Respaldo
cp -r ~/.hydrarouter ~/.hydrarouter.backup

# Restaurar
cp -r ~/.hydrarouter.backup ~/.hydrarouter
```
