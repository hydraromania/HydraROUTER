...
リモートアクセスと本番利用のため、VPSまたはDockerにhydrarouterをデプロイ。
...
| `DATA_DIR` | `~/.hydrarouter` | データベースとデータの保存パス |
...
`/etc/nginx/sites-available/hydrarouter` を作成:
...
# リバースプロキシを使用しない場合、hydrarouterポートを許可
