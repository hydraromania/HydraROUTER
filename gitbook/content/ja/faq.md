...
hydrarouterに関する一般的な質問。

---

## hydrarouterとは?

**hydrarouterは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、hydrarouter自体は100%無料でオープンソースです。**
...
**hydrarouterはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## hydrarouterをセルフホストできますか?

**はい! hydrarouterは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.hydrarouter`)
...
**はい、hydrarouterはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.hydrarouter`(またはカスタム`DATA_DIR`)にローカル保存
- hydrarouterサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**破壊的変更:**
- [CHANGELOG.md](https://github.com/decolua/hydrarouter/blob/main/CHANGELOG.md)を確認
- メジャー更新前に`~/.hydrarouter`をバックアップ
- メジャーバージョンの移行ガイドを確認
...
詳細は[CONTRIBUTING.md](https://github.com/decolua/hydrarouter/blob/main/CONTRIBUTING.md)を参照。

---

## さらにヘルプが必要?

- **ドキュメント:** [hydrarouter.com/docs](https://hydrarouter.com/docs)
