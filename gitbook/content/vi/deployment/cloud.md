...
Triển khai hydrarouter trên VPS hoặc Docker để truy cập từ xa và dùng trong production.
...
| `DATA_DIR` | `~/.hydrarouter` | Đường dẫn lưu database và data |
...
Tạo `/etc/nginx/sites-available/hydrarouter`:
...
# If NOT using reverse proxy, allow hydrarouter ports
...
```bash
# Check if hydrarouter is running
pm2 status
