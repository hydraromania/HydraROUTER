...
Tích hợp hydrarouter với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của hydrarouter.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- hydrarouter đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint hydrarouter:

**Cho hydrarouter cục bộ:**
```
http://localhost:20128/v1
```

**Cho hydrarouter cloud:**
```
https://hydrarouter.com
```

**Các bước:**
1. Trong field **Base URL**, nhập endpoint hydrarouter
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key hydrarouter của bạn
2. Bạn có thể tìm API key trong hydrarouter dashboard tại **Settings → API Keys**
3. Key bắt đầu bằng `sk-hydrarouter-`

### 5. Chọn Model

1. Trong dropdown **Model**, bạn có thể:
   - Chọn từ model có sẵn (nếu Cline auto-detect)
   - Nhập tên model thủ công từ cấu hình hydrarouter
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng hydrarouter để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua hydrarouter
...
3. Nhận giải thích AI qua hydrarouter

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng hydrarouter để hiểu context và thực hiện thay đổi
...
1. Xác minh hydrarouter đang chạy: `curl http://localhost:20128/health`
...
2. Xác minh hydrarouter instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs hydrarouter để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng hydrarouter cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://hydrarouter.com`
2. Đảm bảo bạn đã cấu hình API key trong hydrarouter cloud dashboard
...
## Tích hợp với Tính năng hydrarouter

### Định tuyến Model

hydrarouter tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, hydrarouter tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua hydrarouter dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
