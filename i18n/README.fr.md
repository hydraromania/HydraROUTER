<div align="center">
  <img src="../images/hydrarouter.png?1" alt="Tableau de bord hydrarouter" width="800"/>
  
  # hydrarouter - Routeur IA GRATUIT et économiseur de tokens
...
[🚀 Démarrage rapide](#-démarrage-rapide) • [💡 Fonctionnalités](#-fonctionnalités-clés) • [📖 Configuration](#-guide-dinstallation) • [🌐 Site web](https://hydrarouter.com)
...
Le paquet de ce dépôt est privé (`hydrarouter-app`), donc l'exécution depuis le code source/Docker est le chemin de développement local attendu.
...
> 🎬 **Vous avez réalisé une vidéo sur hydrarouter ?** Envoyez une [Pull Request](https://github.com/decolua/hydrarouter/pulls) en ajoutant votre vidéo à cette section — nous la fusionnerons !

---

## 🛠️ Outils CLI pris en charge

hydrarouter fonctionne parfaitement avec tous les principaux outils de codage IA :
...
Headroom est optionnel et s'exécute séparément. hydrarouter appelle l'endpoint local `/v1/compress` de Headroom, puis conserve le routage, le repli, l'authentification et le suivi d'utilisation normaux :

```
Client → hydrarouter → Headroom /v1/compress → hydrarouter → fournisseur
```
...
Si Headroom est indisponible ou renvoie une erreur, hydrarouter bascule en mode dégradé et envoie la requête originale.
...
- Votre outil CLI envoie le format OpenAI → hydrarouter le traduit → Le fournisseur reçoit le format natif
...
> hydrarouter lui-même **ne vous facture jamais** quoi que ce soit. Vous ne payez que les fournisseurs directement (si vous utilisez des services payants).
>
...
### 📊 Comprendre les coûts et la facturation de hydrarouter

**La réalité de la facturation de hydrarouter :**

✅ **Le logiciel hydrarouter = GRATUIT pour toujours** (open source, ne facture jamais)  
...
❌ **hydrarouter n'envoie jamais de factures** et ne débite pas votre carte
...
- **Fournisseurs bon marché** (GLM, MiniMax) : Payez-les directement, hydrarouter se contente de router
- **Fournisseurs GRATUITS** (iFlow, Kiro, Qwen) : Vraiment gratuits pour toujours, sans frais cachés
- **hydrarouter** : Ne facture jamais rien, jamais
...
<details>
<summary><b>💳 hydrarouter me facturera-t-il ?</b></summary>

**Non.** hydrarouter est un logiciel gratuit et open source qui s'exécute sur votre propre ordinateur. Il ne vous facture jamais rien.

**Vous ne payez que :**

- ✅ **Fournisseurs par abonnement** (Claude Code 20 $/mois, Codex 20 à 200 $/mois) → Payez-les directement sur leurs sites web
- ✅ **Fournisseurs bon marché** (GLM, MiniMax) → Payez-les directement, hydrarouter route simplement vos requêtes
- ❌ **hydrarouter lui-même** → **Ne facture jamais rien, jamais**

hydrarouter est un proxy/routeur local. Il n'a pas votre carte bancaire, ne peut pas envoyer de factures et ne possède aucun système de facturation. C'est un logiciel entièrement gratuit.
...
hydrarouter se contente de router vos requêtes vers eux — il n'y a pas de "piège" ni de facturation future. Ce sont de véritables services gratuits, et hydrarouter les rend faciles à utiliser avec le support du repli.
...
3. **Utilisez les fournisseurs par abonnement en dernier :**
   ...
   - hydrarouter vous aide à maximiser leur valeur grâce au suivi de quota
...
Le repli intelligent de hydrarouter évite les frais surprises :
...
**Vous gardez le contrôle :** Définissez des limites de dépenses par fournisseur dans le tableau de bord, et hydrarouter les respecte.
...
**Astuce pro :** Utilisez Opus pour les tâches complexes et Sonnet pour la vitesse. hydrarouter suit le quota par modèle !
...
Settings → Models → Advanced :
  ...
  OpenAI API Key : [depuis le tableau de bord hydrarouter]
...
```json
{
  ...
  "anthropic_api_key": "votre-clé-api-hydrarouter"
}
```
...
export OPENAI_API_KEY="votre-clé-api-hydrarouter"
...
> **Remarque :** OpenClaw ne fonctionne qu'avec un hydrarouter local. Utilisez `127.0.0.1` au lieu de `localhost` pour éviter les problèmes de résolution IPv6.
...
docker pull decolua/hydrarouter:latest   # mise à jour vers la dernière version
```

**Persistance des données :** `$HOME/.hydrarouter/db/data.sqlite` sur l'hôte ↔ `/app/data/db/data.sqlite` dans le conteneur.

### Variables d'environnement

| Variable                                             | Par défaut                                  | Description                                                                                  |
| ---------------------------------------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                                         | Généré automatiquement (`~/.hydrarouter/jwt-secret`) | Secret de signature JWT pour le cookie d'authentification du tableau de bord (remplacez-le pour partager entre instances) |
| `INITIAL_PASSWORD`                                   | `123456`                                    | Mot de passe de première connexion quand aucun hash n'est enregistré                          |
| `DATA_DIR`                                           | `~/.hydrarouter`                                | Emplacement principal des données de l'app (SQLite dans `$DATA_DIR/db/data.sqlite`)          |
...
| `CLOUD_URL`                                          | `https://hydrarouter.com`                       | URL de base de l'endpoint de synchronisation cloud côté serveur                               |
| `NEXT_PUBLIC_BASE_URL`                               | `http://localhost:3000`                     | URL de base publique/rétrocompatible (préférez `BASE_URL` pour le runtime serveur)            |
| `NEXT_PUBLIC_CLOUD_URL`                              | `https://hydrarouter.com`                       | URL cloud publique/rétrocompatible (préférez `CLOUD_URL` pour le runtime serveur)             |
...
- `${DATA_DIR}` et `~/.hydrarouter` résolvent tous deux le même emplacement dans un conteneur Docker — le lien symbolique `/root/.hydrarouter -> /app/data` est créé au moment de la compilation.
...
- Renouvelé automatiquement par hydrarouter
...
- **Site web** : [hydrarouter.com](https://hydrarouter.com)
- **GitHub** : [github.com/decolua/hydrarouter](https://github.com/decolua/hydrarouter)
- **Problèmes** : [github.com/decolua/hydrarouter/issues](https://github.com/decolua/hydrarouter/issues)

---

## 👥 Contributeurs

Merci à tous les contributeurs qui ont contribué à améliorer hydrarouter !
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Un fork TypeScript complet de hydrarouter. Ajoute plus de 36 fournisseurs, un repli automatique à 4 niveaux, des API multimodales (images, embeddings, audio, TTS), un disjoncteur, un cache sémantique, des évaluations de LLM et un tableau de bord soigné. Plus de 368 tests unitaires. Disponible via npm et Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — économiseur de tokens en Rust. hydrarouter porte son pipeline de compression en JS → **−20 à 40 % de tokens d'entrée** sur chaque requête.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) par **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. hydrarouter adapte son prompt → **−65 % de tokens de sortie**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) par **[@DietrichGebert](https://github.com/DietrichGebert)** — compétence de _"dev sénior fainéant"_. hydrarouter injecte sa hiérarchie YAGNI-d'abord → **moins de tokens, moins de code, des diffs plus courts**.

Un immense merci à ces auteurs — sans leur travail, les fonctionnalités d'économie de tokens de hydrarouter n'existeraient pas. Mettez-leur une ⭐ sur GitHub !
