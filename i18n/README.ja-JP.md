<div align="center">
  ...
  **すべてのAIコーディングツール（Claude Code、Cursor、Antigravity、Copilot、Codex、Gemini、OpenCode、Cline、OpenClaw...）を40以上のAIプロバイダーと100以上のモデルに接続。**
  ...
  [🚀 クイックスタート](#-クイックスタート) • [💡 機能](#-主な機能) • [📖 セットアップ](#-セットアップガイド) • [🌐 ウェブサイト](https://hydrarouter.com)
...
このリポジトリパッケージはプライベート（`hydrarouter-app`）のため、ソース/Docker実行がローカル開発の想定パスです。
...
### 📺 完全セットアップガイド - hydrarouter + Claude Code 無料

[![hydrarouter + Claude Code Setup](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 ステップバイステップのチュートリアルを視聴：**
- ✅ hydrarouterのインストールとセットアップ
...
hydrarouterはすべての主要AIコーディングツールとシームレスに連携します：
...
- CLIツールがOpenAIフォーマットで送信 → hydrarouterが変換 → プロバイダーがネイティブフォーマットで受信
...
> hydrarouter自体は**一切課金しません**。有料サービスを使用する場合のみ、プロバイダーに直接支払います。
>
> **例:** ダッシュボードにiFlowモデルの使用で「合計コスト$290」と表示されている場合、
> これは有料APIを直接使用した場合に支払うであろう金額を表しています。実際のコスト = **$0**（iFlowは無料無制限）。
>
> これは無料モデルやhydrarouter経由のルーティングでどれだけ節約しているかを示す「節約トラッカー」と考えてください！
...
- **格安プロバイダー**（GLM、MiniMax）：直接支払い、hydrarouterはルーティングのみ
...
ダッシュボードはトークン使用量を追跡し、有料APIを直接使用した場合の**推定コスト**を表示します。これは**実際の課金ではなく**、hydrarouterを通じて無料モデルや既存のサブスクリプションを使用することでどれだけ節約しているかを示すための参考値です。
...
<details>
<summary><b>💳 hydrarouterに課金されますか？</b></summary>

**いいえ。** hydrarouterはあなたのコンピューター上で動作する無料のオープンソースソフトウェアです。一切課金しません。

**支払い先：**
- ✅ **サブスクリプションプロバイダー**（Claude Code $20/月、Codex $20-200/月）→ 各ウェブサイトで直接支払い
- ✅ **格安プロバイダー**（GLM、MiniMax）→ 直接支払い、hydrarouterはリクエストをルーティングするだけ
- ❌ **hydrarouter自体** → **一切課金しない**

hydrarouterはローカルプロキシ/ルーターです。クレジットカード情報を持たず、請求書を送信できず、課金システムもありません。完全に無料のソフトウェアです。
...
hydrarouterはリクエストをルーティングするだけで、「罠」や将来の課金はありません。本当に無料のサービスであり、hydrarouterはフォールバックサポートでそれらを使いやすくしています。

**注意:** 一部のサブスクリプションプロバイダー（Antigravity、GitHub Copilot）には無料プレビュー期間があり、後に有料になる可能性がありますが、それはhydrarouterではなく各プロバイダーから明確に告知されます。
...
hydrarouterのスマートフォールバックが予期しない課金を防止します：
...
**あなたがコントロール:** ダッシュボードでプロバイダーごとの支出上限を設定し、hydrarouterはそれを遵守します。
...
**プロのヒント:** 複雑なタスクにはOpus、スピード重視ならSonnet。hydrarouterはモデルごとにクオータを追跡します！
...
設定 → Models → Advanced:
  ...
  OpenAI API Key: [hydrarouterダッシュボードから]
...
> **注意:** OpenClawはローカルのhydrarouterのみで動作します。IPv6解決の問題を避けるため、`localhost` ではなく `127.0.0.1` を使用してください。
...
| 変数 | デフォルト | 説明 |
|------|-----------|------|
| `JWT_SECRET` | 自動生成（`~/.hydrarouter/jwt-secret`） | ダッシュボード認証クッキーのJWT署名シークレット（複数インスタンス間で共有する場合に設定） |
| `INITIAL_PASSWORD` | `123456` | 保存されたハッシュがない場合の初回ログインパスワード |
| `DATA_DIR` | `~/.hydrarouter` | メインアプリのデータベース格納場所（`db.json`） |
...
| `CLOUD_URL` | `https://hydrarouter.com` | サーバーサイドのクラウド同期エンドポイントベースURL |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | 後方互換/公開ベースURL（サーバーランタイムには`BASE_URL`を推奨） |
| `NEXT_PUBLIC_CLOUD_URL` | `https://hydrarouter.com` | 後方互換/公開クラウドURL（サーバーランタイムには`CLOUD_URL`を推奨） |
...
- 使用履歴とログ: `~/.hydrarouter/usage.json` と `~/.hydrarouter/log.txt`、`src/lib/usageDb.js` で管理。
- オプションのリクエスト/トランスレーターログ: `ENABLE_REQUEST_LOGS=true` 時に `<repo>/logs/...`。
- 使用状況ストレージは現在 `~/.hydrarouter` パスロジックに従い、`DATA_DIR` とは独立しています。
...
- **ウェブサイト**: [hydrarouter.com](https://hydrarouter.com)
...
hydrarouterの改善に貢献してくださったすべてのコントリビューターに感謝します！
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — hydrarouterのフルフィーチャーTypeScriptフォーク。36以上のプロバイダー、4段階自動フォールバック、マルチモーダルAPI（画像、埋め込み、音声、TTS）、サーキットブレーカー、セマンティックキャッシュ、LLM評価、洗練されたダッシュボードを追加。368以上のユニットテスト。npmとDockerで利用可能。
