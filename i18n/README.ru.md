<div align="center">
  <img src="../images/hydrarouter.png?1" alt="Панель управления hydrarouter" width="800"/>
  
  # hydrarouter - Free AI Router
  ...
  **Бесплатный AI-провайдер для OpenClaw.**
  ...
  [🚀 Быстрый старт](#-quick-start) • [💡 Возможности](#-key-features) • [📖 Установка](#-setup-guide) • [🌐 Сайт](https://hydrarouter.com)
</div>
...
Пакет этого репозитория приватный (`hydrarouter-app`), поэтому запуск из исходников/Docker — это ожидаемый путь локальной разработки.
...
### 📺 Полное руководство по настройке - hydrarouter + Claude Code БЕСПЛАТНО
  
[![Настройка hydrarouter + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Полное пошаговое руководство:**
- ✅ Установка и настройка hydrarouter
...
hydrarouter бесшовно работает со всеми основными AI-инструментами для кодинга:
...
- Ваш CLI-инструмент отправляет формат OpenAI → hydrarouter транслирует → Провайдер получает родной формат
...
> Сам hydrarouter **никогда ничего не взимает** с вас. Вы платите напрямую провайдерам (если используете платные сервисы).
> 
...
> маршрутизацию через hydrarouter!
...
### 📊 Понимание затрат и оплаты в hydrarouter

**Реальность оплаты hydrarouter:**

✅ **Софт hydrarouter = БЕСПЛАТНО навсегда** (открытый код, никогда не взимает плату)  
...
❌ **hydrarouter никогда не выставляет счёт** и не списывает с вашей карты
...
- **Дешёвые провайдеры** (GLM, MiniMax): Платите им напрямую, hydrarouter только маршрутизирует
- **БЕСПЛАТНЫЕ провайдеры** (iFlow, Kiro, Qwen): Действительно бесплатны навсегда, без скрытых платежей
- **hydrarouter**: Никогда ничего не взимает, никогда
...
Панель отслеживает ваше использование токенов и показывает **оценочные затраты**, как если бы вы напрямую использовали платный API. Это **не реальная оплата** — это справка, показывающая, сколько вы экономите, используя бесплатные модели или существующие подписки через hydrarouter.
...
<details>
<summary><b>💳 Взимает ли с меня плату hydrarouter?</b></summary>

**Нет.** hydrarouter — это бесплатное ПО с открытым кодом, работающее на вашем собственном компьютере. Оно никогда ничего с вас не взимает.

**Вы платите только:**
- ✅ **Провайдерам подписки** (Claude Code $20/мес, Codex $20-200/мес) → Платите им напрямую на их сайте
- ✅ **Дешёвым провайдерам** (GLM, MiniMax) → Платите им напрямую, hydrarouter только маршрутизирует ваши запросы
- ❌ **Самому hydrarouter** → **Никогда ничего не взимает, никогда**

hydrarouter — это локальный прокси/роутер. У него нет вашей кредитной карты, он не может выставлять счета и не имеет платёжной системы. Это полностью бесплатное ПО.
...
hydrarouter только маршрутизирует ваши запросы к ним — никаких «ловушек» или будущих платежей. Это действительно бесплатные сервисы, а hydrarouter облегчает их использование с поддержкой резервирования.

**Примечание:** Некоторые провайдеры подписки (Antigravity, GitHub Copilot) могут иметь бесплатные пробные периоды, которые позже становятся платными, но об этом чётко уведомляют сами провайдеры, а не hydrarouter.
...
3. **Используйте провайдеров подписки в последнюю очередь:**
   ...
   - hydrarouter помогает максимизировать их ценность через отслеживание квоты
...
Умный механизм резервирования hydrarouter предотвращает неожиданные расходы:
...
**Вы контролируете:** Установите лимиты расходов на каждого провайдера в панели, и hydrarouter будет их соблюдать.
...
**Профи-совет:** Используйте Opus для сложных задач, Sonnet для скорости. hydrarouter отслеживает квоту для каждой модели!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [из панели управления hydrarouter]
...
```json
{
  "agents": {
    "defaults": {
      "model": {
        "primary": "hydrarouter/if/glm-4.7"
      }
    }
  },
  "models": {
...
> **Примечание:** OpenClaw работает только с локальным hydrarouter. Используйте `127.0.0.1` вместо `localhost`, чтобы избежать проблем с разрешением имён.
...
# Or use PM2
npm install -g pm2
pm2 start --name hydrarouter -- start
pm2 save
pm2 startup
```
...
docker run -d \
  --name hydrarouter \
  -p 20128:20128 \
  --env-file /root/dev/hydrarouter/.env \
  -v hydrarouter-data:/app/data \
  -v hydrarouter-usage:/root/.hydrarouter \
  hydrarouter
```
...
|----------|---------|-------------|
| `JWT_SECRET` | Автогенерация (`~/.hydrarouter/jwt-secret`) | Секрет подписи JWT для cookie аутентификации панели (задайте для общего доступа между инстансами) |
| `INITIAL_PASSWORD` | `123456` | Пароль первого входа при отсутствии сохранённого хеша |
| `DATA_DIR` | `~/.hydrarouter` | Расположение основной БД приложения (`db.json`) |
...
| `CLOUD_URL` | `https://hydrarouter.com` | Серверный базовый URL эндпоинта облачной синхронизации |
...
| `NEXT_PUBLIC_CLOUD_URL` | `https://hydrarouter.com` | Обратно совместимый/публичный облачный URL (приоритет `CLOUD_URL` для серверного рантайма) |
...
- История использования и логи: `~/.hydrarouter/usage.json` и `~/.hydrarouter/log.txt`, управляется `src/lib/usageDb.js`.
- Опциональные логи запросов/транслятора: `<repo>/logs/...` при `ENABLE_REQUEST_LOGS=true`.
- Хранилище использования следует логике пути `~/.hydrarouter` и независимо от `DATA_DIR`.
...
- Убедитесь, что `CLOUD_URL` указывает на ожидаемый облачный эндпоинт (например, `https://hydrarouter.com`)
...
- Симптом обычно появляется на публичном облачном эндпоинте (`https://hydrarouter.com/v1`) для непотоковых (non-streaming) вызовов.
...
- Локальный рантайм hydrarouter включает резервирование SSE→JSON для непотоковых вызовов, когда upstream возвращает `text/event-stream`.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Отправляет OpenAI-совместимый запрос напрямую на облачный эндпоинт (`https://hydrarouter.com/v1/chat/completions`) с указанной моделью/ключом.
...
- Публичный облачный эндпоинт (`https://hydrarouter.com/v1/chat/completions`):
...
- **Сайт**: [hydrarouter.com](https://hydrarouter.com)
...
Спасибо всем, кто помогает делать hydrarouter лучше!

[![Contributors](https://contrib.rocks/image?repo=decolua/hydrarouter&max=100&columns=20&anon=1)](https://github.com/decolua/hydrarouter/graphs/contributors)
...
См. [Pull Requests](https://github.com/decolua/hydrarouter/pulls) для подробных инструкций.

---

## 🔀 Форки

**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Полнофункциональный TypeScript-форк hydrarouter. Добавляет 36+ провайдеров, авторезервирование на 4 уровнях, мультимодальный API (изображения, embedding, аудио, TTS), circuit breaker, семантическое кеширование, оценку LLM и доработанную панель. 368+ юнит-тестов. Доступен через npm.
