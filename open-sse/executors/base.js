...
export class BaseExecutor {
  ...
  async execute({ model, body, stream, credentials, signal, log, proxyOptions = null, connectTimeoutMs = null }) {
    ...
    for (let urlIndex = 0; urlIndex < fallbackCount; urlIndex++) {
      ...
      const modelLimits = getModelRateLimits(this.provider, model);
      const timeoutMs = connectTimeoutMs || modelLimits?.timeoutMs || this.config?.timeoutMs || FETCH_CONNECT_TIMEOUT_MS;
