...
function createRequestContextResponse() {
  // AgentService asks every run for client context. HydraROUTER has no IDE file
      ...
      try {
        while (!finished) {
          ...
          pending = decodeAgentFrames(pending, (payload) => {
            ...
            // Return an empty context; HydraROUTER is not coupled to an editor.
            if (serverMessage.has(2)) {
              ...
              if (execRequest.has(10)) {
                ...
                // (shell, read, write, …) that HydraROUTER cannot service. Fail the
