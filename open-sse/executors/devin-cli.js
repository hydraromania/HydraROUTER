...
function ensureClientToolsScript() {
  const scriptPath = path.join(os.tmpdir(), "HydraROUTER-devin-client-tools.mjs");
  ...
  async execute({ model, body, credentials, signal, log }) {
    ...
    const sseStream = new ReadableStream({
      start(controller) {
        ...
        // modify the filesystem on the host running HydraROUTER — only expose locally.
        ...
        sendRpc("initialize", {
          protocolVersion: "0.3",
          clientInfo: { name: "HydraROUTER", version: "1.0" },
          capabilities: {},
        });
      },
    });
