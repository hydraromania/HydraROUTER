...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, headroomTimeoutMs, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, sourceFormatOverride, responseFormatOverride, providerThinking }) {
  ...
  // Per-key forced output format (dashboard → API key → outputFormat):
  // request translation still uses the real client format, only the
  // client-facing response is rendered in the forced format.
  const clientFormat = responseFormatOverride || sourceFormat;
  ...
  const activeProxyUrl = proxyOptions.vercelRelayUrl || (proxyOptions.connectionProxyEnabled ? proxyOptions.connectionProxyUrl : "");
  const proxyTracking = activeProxyUrl ? {
    url: activeProxyUrl,
    poolId,
    type: proxyOptions.vercelRelayUrl ? "relay" : "proxy",
    isAuto: isAutoProxy,
  } : null;
  ...
  const streamController = createStreamController({
    onDisconnect: (reason) => {
      ...
      trackRequestError(liveReqId, { error: `Client disconnected: ${reason || "aborted"}`, statusCode: 499 });
      if (onDisconnect) onDisconnect(reason);
    },
    onError: () => {
      trackPendingRequest(model, provider, connectionId, false);
      trackRequestError(liveReqId, { error: "Stream controller error", statusCode: 500 });
    },
    log, provider, model, reqTag
  });

  registerLiveAbort(liveReqId, (err) => streamController.abort(err));
  ...
  if (!providerResponse.ok) {
    ...
    if (statusCode === HTTP_STATUS.BAD_REQUEST) {
      ...
      if (learned) {
        const { body: correctedBody, corrected } = tryAutoCorrect(finalBody || translatedBody, targetFormat, learned.autoCorrect);
        if (corrected) {
          log?.info?.("AUTO_CORRECT", `${provider}/${model} | ${learned.pattern} | retrying with corrected request`);
          trackPendingRequest(model, provider, connectionId, true);
          appendRequestLog({ model, provider, connectionId, status: "RETRY_AUTO_CORRECT" }).catch(() => { });

          const retryResult = await executor.execute({ model, body: correctedBody, stream, credentials, signal: streamController.signal, log, proxyOptions });
          if (retryResult.response.ok) {
            providerResponse = retryResult.response;
            providerUrl = retryResult.url;
            ...
            saveRequestDetail(buildRequestDetail({
              ...
              tokens: { prompt_tokens: 0, completion_tokens: 0 },
              request: extractRequestConfig(body, stream),
              providerRequest: correctedBody,
              response: { error: retryMessage, status: retryStatusCode, thinking: null },
              pxpipe: pxpipeSummary,
              status: "error"
            })).catch(() => { });

            const errMsg = formatProviderError(new Error(retryMessage), provider, model, retryStatusCode);
            if (log?.errorLine) {
              const urlStr = retryResult.url ? `\n    URL: ${retryResult.url}` : "";
              log.errorLine(reqTag, "✗", `ERROR ${retryStatusCode} · ${provider}/${model} · ${Date.now() - requestStartTime}ms${urlStr}\n    ${errMsg}`);
            }
            trackPendingRequest(model, provider, connectionId, false, true);
    ...
    if (!providerResponse.ok) {
      trackPendingRequest(model, provider, connectionId, false, true);
      trackRequestError(liveReqId, { error: message, statusCode });
  ...
  // once WITH the real token usage. Ending it here first used to swallow the
  // token-carrying end (no-op on a settled id) and left error paths that return
  // before those handlers reachable only via the 10-minute stuck sweep — i.e.
  // rows stuck on ACTIVE and, past 45/90s, mislabeled as LENT/BLOCAT.
  const trackDone = () => {
    trackPendingRequest(model, provider, connectionId, false);
  };
  ...
  if (!clientRequestedStreaming && providerRequiresStreaming) {
    const result = await handleForcedSSEToJson({ ...sharedCtx, providerResponse, sourceFormat: clientFormat, targetFormat: providerResponseFormat, customToolNames, trackDone, appendLog });
    if (result) { streamController.handleComplete(); return result; }
  }

  // True non-streaming response
  if (!stream) {
    const result = await handleNonStreamingResponse({ ...sharedCtx, providerResponse, sourceFormat: clientFormat, targetFormat: providerResponseFormat, reqLogger, toolNameMap, customToolNames, trackDone, appendLog });
  ...
  return handleStreamingResponse({ ...sharedCtx, providerResponse, sourceFormat: clientFormat, targetFormat: providerResponseFormat, userAgent, reqLogger, toolNameMap, customToolNames, streamController, onStreamComplete, streamDetailId });
}
