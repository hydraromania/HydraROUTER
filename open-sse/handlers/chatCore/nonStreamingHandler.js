...
import { appendRequestLog, saveRequestDetail, trackRequestEnd, trackRequestError } from "@/lib/usageDb.js";
...
export function translateNonStreamingResponse(responseBody, targetFormat, sourceFormat, customToolNames = null) {
  ...
  if (targetFormat === FORMATS.GEMINI || targetFormat === FORMATS.ANTIGRAVITY || targetFormat === FORMATS.GEMINI_CLI || targetFormat === FORMATS.VERTEX) {
    ...
    // Client speaks Claude (e.g. Claude Code on /v1/messages routed to a
    // Gemini-family upstream): this branch yields an OpenAI intermediate, so
    // convert it onward to a Claude Message instead of leaking chat.completion.
 ...
 */
export async function handleNonStreamingResponse({ providerResponse, provider, model, sourceFormat, targetFormat, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, reqLogger, toolNameMap, customToolNames, trackDone, appendLog, pxpipe, reqTag, log, liveReqId }) {
  trackDone();
  ...
  if (contentType.includes("text/event-stream")) {
    ...
    try {
      ...
      if (liveReqId) trackRequestError(liveReqId, { error: `Invalid JSON response from ${provider}`, statusCode: HTTP_STATUS.BAD_GATEWAY });
