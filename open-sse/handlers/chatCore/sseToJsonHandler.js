...
import { openAICompletionToClaudeMessage } from "../../translator/response/openai-to-claude.js";
import { buildRequestDetail, extractRequestConfig, saveUsageStats, formatDoneLine } from "./requestDetail.js";
import { ROLE, RESPONSES_ITEM } from "../../translator/schema/index.js";

// Responses-API providers (e.g. codex) may emit SSE without content-type + use Responses output shape
const isResponsesProvider = (p) => PROVIDERS[p]?.format === FORMATS.OPENAI_RESPONSES;
import { saveRequestDetail, appendRequestLog, trackRequestEnd, trackRequestError } from "@/lib/usageDb.js";
 ...
 */
export async function handleForcedSSEToJson({ providerResponse, sourceFormat, targetFormat, provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, customToolNames, trackDone, appendLog, reqTag, log, liveReqId }) {
  ...
  if (isCodexResponsesApi) {
    try {
      ...
      if (liveReqId) trackRequestEnd(liveReqId, { tokens: usage, statusCode: 200, status: "completed" });
      ...
      if (liveReqId) trackRequestError(liveReqId, { error: "Failed to convert streaming response to JSON", statusCode: HTTP_STATUS.BAD_GATEWAY });
  ...
  try {
    ...
    if (!parsed) {
      if (liveReqId) trackRequestError(liveReqId, { error: "Invalid SSE response for non-streaming request", statusCode: HTTP_STATUS.BAD_GATEWAY });
      return createErrorResult(HTTP_STATUS.BAD_GATEWAY, "Invalid SSE response for non-streaming request");
    }
    if (parsed.error) {
      if (liveReqId) trackRequestError(liveReqId, { error: parsed.error.message || "Upstream SSE stream failed", statusCode: HTTP_STATUS.BAD_GATEWAY });
    ...
    // A Claude-format client (e.g. Claude Code) behind a forced-streaming
    // provider wants a Message body, not chat.completion.
