...
const QUOTA_ERROR_RE = /quota|rate.?limit|exhausted|too many requests|resource.?exhausted/i;

/** True when the upstream error means this model is spent — try the next fallback model. */
function isQuotaError(status, error) {
  if (status === 429) return true;
  if ((status === 400 || status === 403) && QUOTA_ERROR_RE.test(String(error || ""))) return true;
  return false;
}

/**
 * Execute a chat-search request against the chosen provider.
 * When the provider defines searchViaChat.fallbackModels, quota/rate errors on
 * the requested model automatically retry with the next fallback model.
 * @param {object} params
 * @param {string} params.provider
 * @param {string} params.query
 * @param {number} [params.maxResults]
 * @param {string} [params.model]
 * @param {string[]} [params.fallbackModels] extra models tried in order on quota errors
 * @param {{apiKey?:string, accessToken?:string}} params.credentials
 * @param {{info?:Function, warn?:Function, error?:Function}} [params.log]
 * @returns {Promise<{success:boolean, status?:number, error?:string, data?:object, usedModel?:string}>}
 */
export async function handleChatSearch({
  ...
  const limit =
    Number.isFinite(maxResults) && maxResults > 0
      ...
      : DEFAULT_MAX_RESULTS;
  const primaryModel = model || searchModel(provider);
  const candidates = [primaryModel, ...(fallbackModels || []).filter((m) => m && m !== primaryModel)].filter(Boolean);
  ...
  for (const useModel of candidates) {
    ...
    try {
      ...
      if (err?.name === "AbortError") {
        log?.warn?.(`[chatSearch] timeout provider=${provider} model=${useModel}`);
    ...
    if (!resp.ok) {
      ...
      log?.warn?.(`[chatSearch] upstream error provider=${provider} model=${useModel} status=${resp.status}`);
      lastFailure = { success: false, status: resp.status, error: errText };
      if (isQuotaError(resp.status, errText) && useModel !== candidates[candidates.length - 1]) {
        log?.warn?.(`[chatSearch] ${useModel} exhausted (${resp.status}), trying next fallback model`);
        continue;
      }
      return lastFailure;
    }
  ...
  return lastFailure || { success: false, status: 502, error: "Search failed" };
}
