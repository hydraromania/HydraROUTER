...
/** Wrap a success payload. usedModel = LLM actually billed (chat-search fallback may differ). */
function successResult(data, usedModel) {
  return { success: true, data, ...(usedModel ? { usedModel } : {}), response: jsonResponse(data, 200) };
}
...
export async function handleSearchCore({ body, provider, providerConfig, credentials, log }) {
  ...
  if (providerConfig) {
    ...
    result = await handleChatSearch({
      ...
      fallbackModels: provider.searchViaChat?.fallbackModels,
      credentials,
      log
    });
  } else {
  ...
  if (result.success) return successResult(result.data, result.usedModel);
  ...
  if (
    ...
    if (fallback.success) return successResult(fallback.data, fallback.usedModel);
  }
