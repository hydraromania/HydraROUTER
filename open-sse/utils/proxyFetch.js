 ...
 */
async function getDispatcher(proxyUrl, timeoutMs = null) {
  const normalized = normalizeProxyUrl(proxyUrl);
  if (!normalized) return null;

  const effectiveTimeout = timeoutMs && Number.isFinite(timeoutMs) && timeoutMs > 0 ? timeoutMs : 10_000;
  const cacheKey = `${normalized}::${effectiveTimeout}`;
...
export async function proxyAwareFetch(url, options = {}, proxyOptions = null) {
  ...
  const timeoutMs = proxyOptions?.timeoutMs || null;
  ...
  if (shouldBypassMitmDns(targetUrl)) {
    if (proxyUrl) {
      ...
      try {
        const dispatcher = await getDispatcher(proxyUrl, timeoutMs);
