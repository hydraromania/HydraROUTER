---
name: hydrarouter-image
description: Generate images via hydrarouter /v1/images/generations using OpenAI / Gemini Imagen / DALL-E / FLUX / MiniMax / SDWebUI / ComfyUI / Codex models. Use when the user wants to create, generate, draw, or render an image, picture, or text-to-image (txt2img).
---

# hydrarouter — Image Generation
...
```bash
curl $HYDRAROUTER_URL/v1/models/image | jq '.data[].id'
# Per-model params/options (size enum, quality enum, capabilities like edit)
curl "$HYDRAROUTER_URL/v1/models/info?id=openai/dall-e-3"
```

## Endpoint

`POST $HYDRAROUTER_URL/v1/images/generations`
...
```bash
curl -X POST "$HYDRAROUTER_URL/v1/images/generations?response_format=binary" \
...
```js
const r = await fetch(`${process.env.HYDRAROUTER_URL}/v1/images/generations`, {
  method: "POST",
