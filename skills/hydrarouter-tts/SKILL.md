---
name: hydrarouter-tts
description: Text-to-speech via hydrarouter /v1/audio/speech using OpenAI / ElevenLabs / Deepgram / Edge TTS / Google TTS / Hyperbolic / Inworld voices. Use when the user wants to convert text to speech, generate audio, voiceover, narrate, or read text aloud.
---

# hydrarouter — Text-to-Speech
...
```bash
# 1) List models
curl $HYDRAROUTER_URL/v1/models/tts | jq '.data[].id'
# 2) Per-model metadata (params, voicesUrl if voice-by-id)
curl "$HYDRAROUTER_URL/v1/models/info?id=el/eleven_multilingual_v2"
# 3) List voices (elevenlabs, edge-tts, deepgram, inworld, local-device). Optional ?lang=vi
curl "$HYDRAROUTER_URL/v1/audio/voices?provider=edge-tts&lang=vi" | jq '.data[].model'
```
...
`POST $HYDRAROUTER_URL/v1/audio/speech`
...
```bash
curl -X POST "$HYDRAROUTER_URL/v1/audio/speech" \
...
const r = await fetch(`${process.env.HYDRAROUTER_URL}/v1/audio/speech`, {
  method: "POST",
