---
name: hydrarouter-web-fetch
description: Fetch URL → markdown / text / HTML via hydrarouter /v1/web/fetch using Ollama Cloud / Firecrawl / Jina Reader / Tavily Extract / Exa Contents. Use when the user wants to scrape a webpage, extract URL content, read article, or convert a URL to markdown.
---

# hydrarouter — Web Fetch
...
```bash
curl $HYDRAROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$HYDRAROUTER_URL/v1/models/info?id=firecrawl/fetch"
```
...
`POST $HYDRAROUTER_URL/v1/web/fetch`
...
### Jina Reader
```bash
curl -X POST $HYDRAROUTER_URL/v1/web/fetch \
  -H "Authorization: Bearer $HYDRAROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"jina-reader","url":"https://hydrarouter.com","format":"markdown"}'
```
...
```js
const r = await fetch(`${process.env.HYDRAROUTER_URL}/v1/web/fetch`, {
  method: "POST",
