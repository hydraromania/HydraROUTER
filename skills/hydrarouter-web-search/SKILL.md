---
name: hydrarouter-web-search
description: Web and X search via hydrarouter /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Linkup / SearchAPI / You.com / Perplexity / Xquik. Use when the user wants to search the web, find articles, or search public X posts.
---

# hydrarouter — Web Search
...
```bash
curl $HYDRAROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webSearch") | .id'
# Per-provider params (searchTypes, maxResults, required options like cx for google-pse)
curl "$HYDRAROUTER_URL/v1/models/info?id=tavily/search"
```
...
`POST $HYDRAROUTER_URL/v1/search`
...
```bash
curl -X POST $HYDRAROUTER_URL/v1/search \
  -H "Authorization: Bearer $HYDRAROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"tavily","query":"hydrarouter open source","max_results":5}'
```

JS:

```js
const r = await fetch(`${process.env.HYDRAROUTER_URL}/v1/search`, {
  method: "POST",
  headers: { "Authorization": `Bearer ${process.env.HYDRAROUTER_KEY}`, "Content-Type": "application/json" },
...
Add the Xquik API key in hydrarouter's provider settings. Xquik charges 1 credit per returned post. Continue a search by passing `pagination.next_cursor` as `provider_options.cursor`.
