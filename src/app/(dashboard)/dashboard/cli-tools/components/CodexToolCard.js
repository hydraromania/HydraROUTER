...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_HydraROUTER for localhost if no key, otherwise use selected key
  ...
  const getManualConfigs = () => {
    ...
    const configContent = `# HydraROUTER Configuration for Codex CLI
model = "${selectedModel}"
model_provider = "HydraROUTER"

[model_providers.HydraROUTER]
name = "HydraROUTER"
base_url = "${getEffectiveBaseUrl()}"
wire_api = "responses"

[model_providers.HydraROUTER.http_headers]
