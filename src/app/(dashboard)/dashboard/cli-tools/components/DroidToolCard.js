...
export default function DroidToolCard({
  ...
  const currentBaseUrl = droidStatus?.settings?.customModels?.find((m) => m.id?.startsWith("custom:HydraROUTER"))?.baseUrl || "";

  const getConfigStatus = () => {
    if (!droidStatus?.installed) return null;
    // Check for any HydraROUTER model entry (support multi-model: custom:HydraROUTER-0, custom:HydraROUTER-1, ...)
    const currentConfig = droidStatus.settings?.customModels?.find(m => m.id?.startsWith("custom:HydraROUTER"));
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const existingModels = (droidStatus.settings?.customModels || [])
        .filter(m => m.id?.startsWith("custom:HydraROUTER"))
        .sort((a, b) => (a.index || 0) - (b.index || 0))
        .map(m => m.model);
      if (existingModels.length > 0) {
        setModelList(existingModels);
      } else {
        // Legacy: single model stored as custom:HydraROUTER-0
        const legacy = droidStatus.settings?.customModels?.find(m => m.id === "custom:HydraROUTER-0");
        if (legacy?.model) {
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted">Manual configuration is still available if HydraROUTER is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {droidStatus?.settings?.customModels?.find(m => m.id?.startsWith("custom:HydraROUTER"))?.baseUrl && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!droidStatus?.hasHydraROUTER} loading={restoring}>
