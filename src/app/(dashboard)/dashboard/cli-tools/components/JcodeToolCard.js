...
export default function JcodeToolCard({
  ...
  const currentBaseUrl = jcodeStatus?.config?.providers?.["HydraROUTER"]?.base_url || "";

  const getConfigStatus = () => {
    if (!jcodeStatus?.installed) return null;
    if (!jcodeStatus?.hasHydraROUTER) return "not_configured";
    const currentProvider = jcodeStatus.config?.providers?.["HydraROUTER"];
  ...
  useEffect(() => {
    if (jcodeStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = jcodeStatus.config?.providers?.["HydraROUTER"];
      if (provider) {
  ...
  const handleApplySettings = async () => {
    ...
    try {
      const keyToUse = selectedApiKey?.trim()
        ...
        || (!cloudEnabled ? "sk_HydraROUTER" : null);
  ...
  const getManualConfigs = () => {
    const keyToUse = (selectedApiKey && selectedApiKey.trim())
      ? selectedApiKey
      : (!cloudEnabled ? "sk_HydraROUTER" : "<API_KEY_FROM_DASHBOARD>");

    const configToml = `[providers.HydraROUTER]
type = "openai-compatible"
base_url = "${getEffectiveBaseUrl()}"
auth = "bearer"
api_key_env = "JCODE_9ROUTER_API_KEY"
env_file = "provider-HydraROUTER.env"
default_model = "${selectedModel || "cc/claude-opus-4-7"}"
requires_api_key = true

[[providers.HydraROUTER.models]]
id = "${selectedModel || "cc/claude-opus-4-7"}"`;
    ...
    return [
      ...
      {
        filename: "~/.config/jcode/provider-HydraROUTER.env",
        content: envContent,
      },
    ];
  };
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted mt-2">Manual configuration is still available if HydraROUTER is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {jcodeStatus?.config?.providers?.["HydraROUTER"]?.base_url && (
                ...
                <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                  ...
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile HydraROUTER</code>
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile HydraROUTER --model {selectedModel || "cc/claude-opus-4-7"}</code>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!jcodeStatus?.hasHydraROUTER} loading={restoring}>
