...
export default function APIPageClient({ machineId }) {
  ...
  const [newKeyFormat, setNewKeyFormat] = useState("auto");
  const [editingKeyModels, setEditingKeyModels] = useState(null); // { id, name, models: [] }
  const [showModelSelectFor, setShowModelSelectFor] = useState(null); // "new" | "edit"
 ...
 // Cloudflare Tunnel state
  ...
  const handleCreateKey = async () => {
    ...
    try {
      const res = await fetch("/api/keys", {
        ...
        body: JSON.stringify({
          name: newKeyName,
          allowedModels: newKeyModels.length > 0 ? newKeyModels : null,
          outputFormat: newKeyFormat,
        }),
      });
      const data = await res.json();
  ...
  const handleSaveEditModels = async () => {
    ...
    try {
      const res = await fetch(`/api/keys/${editingKeyModels.id}`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          allowedModels: editingKeyModels.models.length > 0 ? editingKeyModels.models : null,
        }),
      });
      if (res.ok) {
        setKeys(prev => prev.map(k => k.id === editingKeyModels.id ? { ...k, allowedModels: editingKeyModels.models.length > 0 ? editingKeyModels.models : null } : k));
  ...
  const handleUpdateKeyFormat = async (id, outputFormat) => {
    try {
      const res = await fetch(`/api/keys/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        setKeys(prev => prev.map(k => k.id === id ? { ...k, outputFormat } : k));
      }
    } catch (error) {
      console.log("Error updating key format:", error);
    }
  };
  ...
  const handleSetDefaultKey = async (id) => {
    try {
      ...
      if (res.ok) {
        setKeys(prev => prev.map(k => k.id === id ? { ...k, isDefault: true } : { ...k, isDefault: false }));
      }
    } catch (error) {
  ...
  const handleMoveKey = async (id, direction) => {
    ...
    const reordered = updatedKeys.map((k, idx) => ({ ...k, priority: idx }));
    setKeys(reordered);

    // Save priorities in the backend
    try {
      await Promise.all(
        reordered.map((k) =>
          fetch(`/api/keys/${k.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ priority: k.priority }),
          })
        )
      );
    } catch (error) {
      console.log("Error saving key order:", error);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-8">
      {/* Endpoint Card */}
      <Card>
        <div className="glass-panel p-5 rounded-2xl flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border border-border/60 shadow-sm mb-6">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <div className="size-7 rounded-lg bg-brand-500/10 text-brand-500 flex items-center justify-center glow-brand">
                <span className="material-symbols-outlined text-[18px]">api</span>
              </div>
              <h2 className="text-base font-bold text-text-main">API Endpoint</h2>
            </div>
            <p className="text-xs text-text-muted">Configure local, tunnel, and Tailscale endpoints</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          {/* Local */}
          <div className="glass-panel p-3 rounded-xl border border-border/60">
            <EndpointRow
              label="Local"
              url={currentEndpoint}
              copyId="local_url"
          ...
          <div className="glass-panel p-3 rounded-xl border border-border/60">
            <div className="flex items-center gap-3">
              <span className={`text-xs font-mono px-2.5 py-1 rounded-lg shrink-0 min-w-[90px] text-center font-medium border ${
                tunnelEnabled
                  ? "bg-primary/10 text-primary border-primary/20 glow-brand"
                  : "bg-surface-2 text-text-muted border-border/40"
              }`}>Tunnel</span>
      ...
      <Card id="require-api-key">
        <div className="glass-panel p-5 rounded-2xl flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border border-border/60 shadow-sm mb-6">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="text-base font-bold text-text-main">API Keys</h2>
            </div>
            <p className="text-xs text-text-muted">Manage authentication keys and granular model access</p>
          </div>
          <Button icon="add" onClick={() => setShowAddModal(true)} className="glow-brand shadow-sm">
            Create Key
          </Button>
        </div>

        <div className="glass-panel p-4 rounded-xl border border-border/60 flex items-center justify-between mb-6">
          <div>
            <p className="text-sm font-semibold text-text-main">Require API key</p>
        ...
        {keys.length === 0 ? (
          <div className="glass-panel rounded-2xl border border-border/60 text-center py-12 px-4">
            <div className="inline-flex items-center justify-center size-14 rounded-2xl bg-brand-500/10 text-brand-500 mb-3 glow-brand">
            ...
            <Button icon="add" onClick={() => setShowAddModal(true)} className="glow-brand">
              Create Key
            </Button>
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            {keys.map((key, idx) => {
              const hasRestrictedModels = Array.isArray(key.allowedModels) && key.allowedModels.length > 0;
              return (
                <div
                  key={key.id}
                  className={`group glass-panel p-4 rounded-xl border border-border/60 flex items-start sm:items-center justify-between gap-3 transition-all hover:border-brand-500/30 ${key.isActive === false ? "opacity-60" : ""}`}
                >
                  {/* Handle buttons to reorder keys */}
                  <div className="flex flex-col gap-1 mr-1">
                    <button
                      onClick={() => handleMoveKey(key.id, "up")}
                      disabled={idx === 0}
                      className="p-1 rounded bg-surface-2/60 text-text-muted hover:text-brand-500 disabled:opacity-30 disabled:pointer-events-none transition-all cursor-pointer"
                      title="Move Up"
                    >
                  ...
                  <div className="flex-1 min-w-0 pr-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-bold text-text-main flex items-center gap-1.5">
                        {key.name}
                        {key.isDefault ? (
                          <span className="inline-flex items-center gap-0.5 text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 font-bold uppercase tracking-wider glow-emerald">
                            Default
                          </span>
                        ) : (
                          <button
                            onClick={() => handleSetDefaultKey(key.id)}
                            className="hidden group-hover:inline-flex text-[10px] px-2 py-0.5 rounded-full border border-border/60 bg-surface-2/60 text-text-muted hover:text-brand-500 hover:border-brand-500/50 transition-all font-semibold uppercase tracking-wider cursor-pointer"
                          >
                            Set Default
                          </button>
                        )}
                      ...
                      <button
                        onClick={() => setEditingKeyModels({ id: key.id, name: key.name, models: key.allowedModels || [] })}
                        className={`text-xs px-2.5 py-0.5 rounded-full border flex items-center gap-1 transition-all cursor-pointer ${
                          hasRestrictedModels
                            ? "bg-brand-500/10 text-brand-500 border-brand-500/20 hover:bg-brand-500/20 glow-brand"
                            : "bg-surface-2/80 text-text-muted border-border/60 hover:border-brand-500/50"
                        }`}
                        title="Configure model access"
                      >
                        <span className="material-symbols-outlined text-[13px]">tune</span>
                        {hasRestrictedModels ? `${key.allowedModels.length} models allowed` : "All models allowed"}
                      </button>
                      <select
                        value={key.outputFormat || "auto"}
                        onChange={(e) => handleUpdateKeyFormat(key.id, e.target.value)}
                        className="text-xs px-2.5 py-0.5 rounded-full border bg-surface-2/80 text-text-muted border-border/60 hover:border-brand-500/50 transition-colors cursor-pointer"
                        title="Force response format: Auto detects from request, OpenAI/Claude always answer in that shape"
                      >
                        <option value="auto">Auto format</option>
                        <option value="openai">OpenAI out</option>
                        <option value="claude">Claude out</option>
                      </select>
                    </div>
                    <div className="flex items-center gap-2 mt-2">
                      <code className="text-xs text-text-muted font-mono bg-bg/50 px-2 py-0.5 rounded border border-border/40">
                        {visibleKeys.has(key.id) ? key.key : maskKey(key.key)}
                      </code>
                      <button
                        onClick={() => toggleKeyVisibility(key.id)}
                        className="p-1 hover:bg-brand-500/10 rounded-lg text-text-muted hover:text-brand-500 transition-all cursor-pointer"
                    ...
                    {hasRestrictedModels && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {key.allowedModels.slice(0, 5).map((m) => (
                          <span key={m} className="text-[10px] px-2 py-0.5 rounded-md bg-surface-2/80 text-text-muted border border-border/60 font-mono">
                            {m}
                          </span>
                        ))}
                        {key.allowedModels.length > 5 && (
                          <span className="text-[10px] px-2 py-0.5 rounded-md bg-surface-2/80 text-text-muted border border-border/60 font-mono">
                            +{key.allowedModels.length - 5} more
                          </span>
                        )}
                      </div>
                    )}
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      {key.isActive === false && (
                        <span className="text-[10px] font-semibold text-amber-500 px-2 py-0.5 rounded bg-amber-500/10 border border-amber-500/20">Paused</span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      onClick={() => setEditingKeyModels({ id: key.id, name: key.name, models: key.allowedModels || [] })}
                      className="p-2 hover:bg-brand-500/10 rounded-lg text-text-muted hover:text-brand-500 transition-all cursor-pointer"
                    ...
                    <button
                      ...
                      className="p-2 hover:bg-red-500/10 rounded-lg text-red-500 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all cursor-pointer"
                    >
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div>
            ...
            <select
              value={newKeyFormat}
              onChange={(e) => setNewKeyFormat(e.target.value)}
              className="mt-1.5 w-full text-sm px-3 py-2 rounded-lg border bg-surface-2 text-text-main border-border"
            >
              <option value="auto">Auto (detect from request)</option>
              <option value="openai">Force OpenAI output</option>
              <option value="claude">Force Claude output</option>
            </select>
            <p className="text-xs text-text-muted mt-1">
              Force Claude output for Claude Code keys so any upstream answers in Claude Message shape.
            </p>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button
                type="button"
                onClick={() => setShowModelSelectFor("new")}
                className="text-xs text-primary hover:underline flex items-center gap-1"
              >
                <span className="material-symbols-outlined text-sm">add</span>
                Select Models / Combos
              </button>
            </div>
            {newKeyModels.length === 0 ? (
              <p className="text-xs text-text-muted italic bg-surface-2 p-2.5 rounded-lg border border-border">
                All models allowed (no restrictions). Click "Select Models / Combos" to restrict access.
              </p>
            ) : (
              <div className="flex flex-wrap gap-1.5 max-h-40 overflow-y-auto p-2 bg-surface-2 rounded-lg border border-border">
                {newKeyModels.map((m) => (
                  <span
                    key={m}
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => setNewKeyModels(newKeyModels.filter((item) => item !== m))}
      ...
      {/* Edit Model Access Modal */}
      <Modal
        isOpen={!!editingKeyModels}
        title={`Edit Model Access: ${editingKeyModels?.name || ""}`}
      ...
      >
        <div className="flex flex-col gap-4">
          <div>
            ...
            {editingKeyModels?.models?.length === 0 ? (
              <p className="text-xs text-text-muted italic bg-surface-2 p-2.5 rounded-lg border border-border">
                All models allowed (no restrictions). Click "Add Models / Combos" to restrict access.
              </p>
            ) : (
              <div className="flex flex-wrap gap-1.5 max-h-48 overflow-y-auto p-2 bg-surface-2 rounded-lg border border-border">
                {editingKeyModels?.models?.map((m) => (
                  <span
                    key={m}
                    className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary border border-primary/20"
                  >
                    ...
                    <button
                      ...
                      onClick={() =>
                        setEditingKeyModels({
                          ...editingKeyModels,
                          models: editingKeyModels.models.filter((item) => item !== m),
                        })
                      }
                      className="hover:text-red-500 rounded-full"
                    >
          ...
          <div className="flex gap-2">
            <Button onClick={handleSaveEditModels} fullWidth>
              Save
            </Button>
            <Button
      ...
      {/* ModelSelectModal for API Key allowed models */}
      <ModelSelectModal
        isOpen={!!showModelSelectFor}
        onClose={() => setShowModelSelectFor(null)}
        onSelect={(model) => {
          const val = model.value;
          if (showModelSelectFor === "new") {
            if (!newKeyModels.includes(val)) {
              setNewKeyModels([...newKeyModels, val]);
            }
          } else if (showModelSelectFor === "edit" && editingKeyModels) {
            if (!editingKeyModels.models.includes(val)) {
              setEditingKeyModels({
                ...editingKeyModels,
                models: [...editingKeyModels.models, val],
              });
            }
          }
        }}
        onDeselect={(model) => {
          const val = model.value;
          if (showModelSelectFor === "new") {
            setNewKeyModels(newKeyModels.filter((m) => m !== val));
          } else if (showModelSelectFor === "edit" && editingKeyModels) {
            setEditingKeyModels({
              ...editingKeyModels,
              models: editingKeyModels.models.filter((m) => m !== val),
            });
          }
        }}
        ...
        addedModelValues={
          showModelSelectFor === "new"
            ? newKeyModels
            : editingKeyModels?.models || []
        }
        closeOnSelect={false}
        title="Select Allowed Models & Combos"
      />
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-text-muted">
                  Expose your local HydraROUTER to the internet. No port forwarding, no static IP needed. Share endpoint URL with your team or use it in Cursor, Cline, and other AI tools from anywhere.
                </p>
              </div>
            </div>
          </div>
      ...
      {showModelSelectFor && (
        <ModelSelectModal
          ...
          onSelect={(item) => {
            ...
            if (showModelSelectFor === "new") {
              if (!newKeyModels.includes(val)) setNewKeyModels([...newKeyModels, val]);
            } else if (showModelSelectFor === "edit" && editingKeyModels) {
              const current = editingKeyModels.models || [];
