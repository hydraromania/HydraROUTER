...
import { MEDIA_PROVIDER_KINDS, AI_PROVIDERS, getProviderAlias, resolveProviderId } from "@/shared/constants/providers";
...
export function GenericExampleCard({ providerId, kind }) {
  ...
  const isChatBasedWeb = (kind === "webSearch" && AI_PROVIDERS[providerId]?.searchViaChat) ||
    (kind === "webFetch" && AI_PROVIDERS[providerId]?.fetchViaChat);
  const kindModels = isChatBasedWeb
    ? getModelsByProviderId(providerId).filter((m) => !getModelKind(m) || getModelKind(m) === "llm")
    : getModelsByProviderId(providerId).filter((m) => getModelKind(m) === kind);
  // Kinds that need a model identifier in the request (image/video/music/imageToText or webSearch/webFetch with chat models)
  const KIND_NEEDS_MODEL = new Set(["image", "video", "music", "imageToText"]);
  const needsModel = KIND_NEEDS_MODEL.has(kind) || ((kind === "webSearch" || kind === "webFetch") && kindModels.length > 0);
  const allowManualModel = needsModel && kindModels.length === 0;
  const defaultModelForChat = kind === "webFetch"
    ? AI_PROVIDERS[providerId]?.fetchViaChat?.defaultModel
    : AI_PROVIDERS[providerId]?.searchViaChat?.defaultModel;
  const initialModel = kindModels.find((m) => m.id === defaultModelForChat)?.id ?? kindModels[0]?.id ?? "";
