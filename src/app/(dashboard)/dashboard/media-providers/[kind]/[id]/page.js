...
export default function MediaProviderDetailPage() {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {/* Models - hidden for tts or pure searchConfig/fetchConfig providers (provider IS the model); custom uses prefix as alias */}
      {kind !== "tts" && (kind !== "webFetch" || !!provider.fetchViaChat) && (kind !== "webSearch" || !!provider.searchViaChat) && (
        <ModelsCard
          providerId={id}
          kindFilter={(kind === "webSearch" || kind === "webFetch") ? "llm" : kind}
          providerAliasOverride={isCustom ? customNode?.prefix : undefined}
        />
      )}

      {/* Provider Info — config-driven, supports searchConfig, fetchConfig, ttsConfig, embeddingConfig, searchViaChat, fetchViaChat */}
      {!isCustom && (provider.searchConfig || provider.fetchConfig || provider.ttsConfig || provider.sttConfig || provider.embeddingConfig || provider.searchViaChat || provider.fetchViaChat) && (
        <ProviderInfoCard
          config={
            kind === "webFetch" ? (provider.fetchConfig || { mode: "chat-completions", defaultModel: provider.fetchViaChat?.defaultModel, pricingUrl: provider.fetchViaChat?.pricingUrl, freeTier: provider.fetchViaChat?.freeTier })
