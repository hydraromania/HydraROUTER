  ...
  return (
    <div className={`group min-w-0 max-w-full rounded-lg border px-3 py-2 ${borderColor} hover:bg-sidebar/50`}>
      <div className="flex min-w-0 items-start gap-2 sm:items-center">
        ...
        >
          {isBlocked ? "block" : testStatus === "ok" ? "check_circle" : testStatus === "error" ? "cancel" : "smart_toy"}
        </span>
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {isBlocked && (
              <span className="inline-flex items-center gap-1 rounded bg-amber-500/15 px-1.5 py-0.5 text-[10px] font-medium text-amber-600 dark:text-amber-400" title={blockedInfo?.blockedUntil ? `Blocked until ${new Date(blockedInfo.blockedUntil).toLocaleTimeString()}` : "Blocked due to repeated timeout/503/504 errors"}>
                Blocked
              </span>
            )}
          </div>
        ...
        {isBlocked && onUnblock && (
          <div className="relative shrink-0 group/btn">
            <button
              ...
              className="rounded p-0.5 text-amber-500 hover:bg-amber-500/10 hover:text-amber-600 transition-colors"
              title="Unblock model"
            >
              <span className="material-symbols-outlined text-sm" style={isUnblocking ? { animation: "spin 1s linear infinite" } : undefined}>
                {isUnblocking ? "progress_activity" : "lock_open"}
              </span>
            </button>
...
ModelRow.propTypes = {
  ...
  blockedInfo: PropTypes.object,
  onUnblock: PropTypes.func,
  isUnblocking: PropTypes.bool,
};
