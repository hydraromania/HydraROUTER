...
export default function ProviderDetailPage() {
  ...
  const fetchBlockedModels = useCallback(async () => {
    try {
  ...
  const handleUnblockModel = async (fullModelStr) => {
    if (unblockingModels.has(fullModelStr)) return;
    setUnblockingModels((prev) => new Set(prev).add(fullModelStr));
    try {
      const res = await fetch("/api/usage/blocked-models", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "unblock", model: fullModelStr }),
      });
      if (res.ok) {
        await fetchBlockedModels();
      }
    } catch (error) {
      console.log("Error unblocking model:", error);
    } finally {
      setUnblockingModels((prev) => {
        const next = new Set(prev);
        next.delete(fullModelStr);
        return next;
      });
    }
  };

  const isModelCurrentlyBlocked = (modelId) => {
    if (!blockedModels || blockedModels.length === 0) return null;
    const candidates = [
      modelId,
      `${providerStorageAlias}/${modelId}`,
      `${providerDisplayAlias}/${modelId}`,
      `${providerId}/${modelId}`,
    ];
  ...
  useEffect(() => {
    ...
    fetchDisabledModels();
    fetchBlockedModels();
  }, [fetchConnections, fetchAliases, fetchCustomModels, fetchDisabledModels, fetchBlockedModels]);
  ...
  const handleSwapPriority = async (index1, index2) => {
    if (index1 < 0 || index2 < 0 || index1 >= connections.length || index2 >= connections.length) return;
    const newConnections = [...connections];
    const [moved] = newConnections.splice(index1, 1);
    newConnections.splice(index2, 0, moved);
    const reordered = newConnections.map((c, i) => ({ ...c, priority: i + 1 }));
    ...
    try {
      await fetch("/api/providers/reorder", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ orderedIds: reordered.map((c) => c.id), provider: providerId }),
      });
    } catch (error) {
  ...
  const handleDragEnd = async (event) => {
    ...
    const newConnections = arrayMove(connections, oldIndex, newIndex).map((c, i) => ({
      ...c,
      priority: i + 1,
    }));
    ...
    try {
      await fetch("/api/providers/reorder", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ orderedIds: newConnections.map((c) => c.id), provider: providerId }),
      });
    } catch (error) {
...
function SortableConnectionItem({
  ...
  const { attributes, listeners, setNodeRef, transform, isDragging } = useSortable({ id: conn.id });
  const style = {
  ...
  const connectionsList = (
    ...
    >
      <SortableContext items={connections.map((c) => c.id)} strategy={verticalListSortingStrategy}>
        <div className="flex min-w-0 flex-col gap-2 max-h-[550px] overflow-y-auto pr-1">
          {connections.map((conn, index) => (
            <SortableConnectionItem
              ...
              onAutoPingToggle={(on) => handleAutoPingConnection(conn.id, on)}
    ...
    >
      <div className="flex flex-col gap-3">
        <div className="flex flex-col">
          ...
          >
            <span className="material-symbols-outlined text-text-muted text-[18px]">autorenew</span>
            <span className="text-sm text-text-main">Auto (rotate all)</span>
          </button>
          )}
          {proxyPools.map((pool) => (
            <button
    ...
    return (
      <div className="flex flex-wrap gap-3">
        ...
        {customModelRows.map((model) => {
          const blockedInfo = isModelCurrentlyBlocked(model.id);
          ...
          return (
            <ModelRow
              ...
              isUnblocking={unblockingModels.has(blockedInfo?.model || `${providerStorageAlias}/${model.id}`)}
            />
          );
        })}
        ...
        {displayModels.map((model) => {
          ...
          return (
            <ModelRow
              ...
              isUnblocking={unblockingModels.has(blockedInfo?.model || fullModel)}
            />
          );
        })}
        ...
        <Card>
          ...
          {connections.length === 0 ? (
            ...
            <>
              ...
              {connections.length > 0 && (
                <div className="mb-3 flex items-center justify-between gap-2 border-b border-border/40 pb-2.5 text-xs text-text-muted">
                  <label className="flex cursor-pointer items-center gap-2 hover:text-primary transition-colors">
                    ...
                    />
                    <span className="font-medium">Select All ({connections.length})</span>
                  </label>
                  <div className="flex items-center gap-1.5 text-[11px] text-text-muted/70">
                    <span className="material-symbols-outlined text-[14px]">drag_indicator</span>
                    <span>Trage elementele sau folosește săgețile pentru a reordona prioritatea</span>
                  </div>
                </div>
              )}
              {connectionsList}
