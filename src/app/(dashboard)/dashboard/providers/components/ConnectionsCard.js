...
function ConnectionRow({ connection, proxyPools, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onUpdateProxy, onEdit, onDelete }) {
  ...
  const proxyBadgeVariant = isAutoProxy || boundProxyPool?.isActive === true ? "success" : (boundProxyPoolId || hasLegacyProxy) ? "error" : "default";
      ...
      <div className="flex w-full min-w-0 flex-1 items-start gap-3 sm:items-center">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-1">
            ...
            {hasAnyProxy && <Badge variant={proxyBadgeVariant} size="sm">{isAutoProxy ? "Auto" : "Proxy"}</Badge>}
            ...
            <div className="relative" ref={proxyDropdownRef}>
              ...
              {showProxyDropdown && (
                <div className="absolute right-0 top-full mt-1 z-50 bg-bg border border-border rounded-lg shadow-lg py-1 min-w-[160px]">
                  ...
                  {showAutoProxy && (
                  <button onClick={() => handleSelectProxy("__auto__")} title="Rotate automatically through all active proxies. A proxy is paused 3h after 3 consecutive 429s." className={`w-full text-left px-3 py-1.5 text-sm hover:bg-black/5 dark:hover:bg-white/5 ${isAutoProxy ? "text-primary font-medium" : "text-text-main"}`}>Auto (rotate)</button>
                  )}
                  {(proxyPools || []).map((pool) => (
