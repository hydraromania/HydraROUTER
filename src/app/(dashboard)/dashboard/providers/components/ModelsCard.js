...
export function ModelRow({ model, fullModel, copied, onCopy, testStatus, isCustom, isFree, onDeleteAlias, onTest, isTesting }) {
  ...
  const isEmbedding = model?.kind === "embedding" || model?.type === "embedding" || model?.dimensions != null;
    ...
    <div className={`group px-3 py-2 rounded-lg border ${borderColor} hover:bg-sidebar/50`}>
      <div className="flex items-center gap-2">
        <span className="material-symbols-outlined text-base" style={iconColor ? { color: iconColor } : undefined}>
          {testStatus === "ok" ? "check_circle" : testStatus === "error" ? "cancel" : isEmbedding ? "data_array" : "smart_toy"}
        </span>
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center gap-1.5 flex-wrap">
            <code className="text-xs text-text-muted font-mono bg-sidebar px-1.5 py-0.5 rounded">{fullModel}</code>
            {model.dimensions && (
              <span className="text-[10px] font-medium font-mono text-cyan-500 bg-cyan-500/10 border border-cyan-500/20 px-1.5 py-0.2 rounded">
