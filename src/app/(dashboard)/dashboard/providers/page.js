...
export default function ProvidersPage() {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:px-0">
      <div className="flex items-center justify-between gap-4 glass-panel p-2.5 px-4 rounded-xl">
        <div className="flex items-center gap-2 text-xs font-semibold text-text-muted">
          <span className="material-symbols-outlined text-[18px] text-brand-500">tune</span>
          <span>Filter Status</span>
        </div>
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value)}
          className="h-8 rounded-lg border border-border/60 bg-surface/70 px-3 text-xs text-text-main font-medium outline-none transition-all hover:bg-surface focus:border-brand-500 cursor-pointer shadow-2xs"
...
function ProviderCard({ providerId, provider, stats, authType, onToggle }) {
  ...
  return (
    <Link href={`/dashboard/providers/${providerId}`} className="group min-w-0 block">
      <Card
        padding="xs"
        className={`h-full border border-border/60 hover:border-brand-500/40 hover:shadow-md transition-all duration-200 cursor-pointer ${allDisabled ? "opacity-50" : ""}`}
      >
        <div className="flex min-w-0 items-center justify-between gap-3 p-1">
          <div className="flex min-w-0 items-center gap-3">
            <div
              className="size-9 shrink-0 rounded-xl flex items-center justify-center p-1 shadow-2xs border border-white/10"
              style={{
            ...
            <div className="min-w-0">
              <h3 className="truncate font-semibold text-sm group-hover:text-brand-500 transition-colors">{provider.name}</h3>
