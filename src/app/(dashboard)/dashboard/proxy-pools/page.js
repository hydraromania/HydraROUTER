...
function getAutoPauseRemainingMs(pool) {
  if (!pool?.autoPausedUntil) return 0;
  const diff = new Date(pool.autoPausedUntil).getTime() - Date.now();
  return Number.isFinite(diff) && diff > 0 ? diff : 0;
}

function formatAutoPauseRemaining(ms) {
...
export default function ProxyPoolsPage() {
  ...
  const handleResumeAutoPause = async (pool) => {
    try {
      const res = await fetch(`/api/proxy-pools/${pool.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ autoPausedUntil: null, autoConsecutive429: 0 }),
      });
      if (!res.ok) {
        notify.error("Failed to resume proxy");
        return;
      }
      await fetchProxyPools();
      notify.success("Proxy resumed for auto rotation");
    } catch (error) {
      console.log("Error resuming auto pause:", error);
      notify.error("Failed to resume proxy");
    }
  };
              ...
              <div key={pool.id} className="flex flex-col gap-3 py-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {getAutoPauseRemainingMs(pool) > 0 && (
                      <Badge variant="warning" size="sm" dot>
                        auto-paused {formatAutoPauseRemaining(getAutoPauseRemainingMs(pool))}
                      </Badge>
                    )}
                  </div>
