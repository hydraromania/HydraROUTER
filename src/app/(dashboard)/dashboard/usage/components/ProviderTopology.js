...
function ProviderNode({ data }) {
  ...
  return (
    <div
      className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl border transition-all duration-300 backdrop-blur-md ${
        active
          ? "bg-surface shadow-lg scale-105"
          : "bg-surface/90 hover:bg-surface border-border-subtle hover:border-border hover:shadow-sm"
      }`}
      style={{
        borderColor: active ? color : undefined,
        boxShadow: active ? `0 0 20px ${color}35, 0 4px 12px rgba(0,0,0,0.05)` : undefined,
        minWidth: "150px",
      }}
    >
      ...
      {active && (
        <span className="relative ml-auto flex h-2.5 w-2.5 shrink-0">
          ...
          <span className="relative inline-flex rounded-full h-2.5 w-2.5 shadow-sm" style={{ backgroundColor: color }} />
        </span>
      )}
    </div>
  );
...
function RouterNode({ data }) {
  ...
  return (
    <div
      className={`relative z-[1] flex items-center justify-center px-5 py-3 rounded-2xl border transition-all duration-300 shadow-md backdrop-blur-md min-w-[140px] ${
        powering
          ? "topology-router-core border-brand-500 bg-gradient-to-br from-brand-500/25 via-amber-400/20 to-brand-600/30 shadow-[0_0_25px_rgba(229,106,74,0.35)]"
          : "border-border-subtle bg-surface/90 hover:border-brand-500/40"
      }`}
    >
      ...
      <div className="w-7 h-7 mr-2.5 flex items-center justify-center rounded-lg bg-brand-500/20 text-brand-500 font-black text-xs shadow-inner">
        ⚡
      </div>
      <span className={`text-sm font-bold tracking-tight ${powering ? "text-brand-500 dark:text-brand-400" : "text-text-main"}`}>
        HydraROUTER
      </span>
      {data.activeCount > 0 && (
        <span className="ml-2.5 px-2 py-0.5 rounded-full bg-brand-500 text-white text-[11px] font-bold shadow-sm animate-pulse">
          {data.activeCount}
        </span>
      )}
    </div>
...
export default function ProviderTopology({ providers = [], activeRequests = [], lastProvider = "", errorProvider = "" }) {
  ...
  return (
    <div ref={containerRef} className="relative h-[340px] w-full min-w-0 overflow-hidden rounded-2xl border border-border-subtle bg-surface/80 shadow-sm backdrop-blur-md sm:h-[480px]">
      <div className="absolute top-3 left-3 z-10 flex items-center gap-2 rounded-lg bg-bg/80 px-2.5 py-1 text-xs font-medium text-text-muted border border-border/50 backdrop-blur-sm">
        <span className="material-symbols-outlined text-[15px] text-brand-500">hub</span>
