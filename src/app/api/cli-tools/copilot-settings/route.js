...
const hasHydraROUTERConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "HydraROUTER");
};

const getHydraROUTEREntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "HydraROUTER") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getHydraROUTEREntry(config);
...
// POST - Apply HydraROUTER config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing HydraROUTER entry or append
    const idx = config.findIndex((e) => e.name === "HydraROUTER");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove HydraROUTER entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "HydraROUTER");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "HydraROUTER removed from Copilot config",
    });
  } catch (error) {
