...
// Check if settings has HydraROUTER customModels
const hasHydraROUTERConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:HydraROUTER"));
};
...
// POST - Update HydraROUTER customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing HydraROUTER configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:HydraROUTER"));
...
// DELETE - Remove HydraROUTER customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove HydraROUTER customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:HydraROUTER"));
