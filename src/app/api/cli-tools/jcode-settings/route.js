...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-HydraROUTER.env");
};
...
const hasHydraROUTERConfig = (config) => {
  ...
  if (providers["HydraROUTER"]) return true;
...
export async function GET() {
  ...
  const hasHydraROUTER = hasHydraROUTERConfig(config);
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile HydraROUTER",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["HydraROUTER"];
    ...
    return NextResponse.json({
      success: true,
      message: "HydraROUTER configuration removed from jcode",
    });
  } catch (error) {
