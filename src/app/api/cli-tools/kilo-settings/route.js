...
const hasHydraROUTERConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["HydraROUTER"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("HydraROUTER");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasHydraROUTER: hasHydraROUTERConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "HydraROUTER", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "HydraROUTER settings removed from Kilo Code" });
  } catch (error) {
