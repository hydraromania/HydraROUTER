...
// Check if settings has HydraROUTER config
const hasHydraROUTERConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["HydraROUTER"];
};

// Read per-agent models.json and return current model id (without "HydraROUTER/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["HydraROUTER"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasHydraROUTER: hasHydraROUTERConfig(settings),
...
const writeAgentModels = async (agentDir, model, baseUrl, apiKey) => {
  ...
  existing.providers["HydraROUTER"] = {
    baseUrl,
...
// POST - Update HydraROUTER settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `HydraROUTER/${model}`;

    // Remove all old HydraROUTER/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("HydraROUTER/"))
    ...
    // Add fresh HydraROUTER models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`HydraROUTER/${m}`] = {};
    });

    // Remove old HydraROUTER model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("HydraROUTER/")) {
    ...
    // Update models.providers.HydraROUTER with all models
    settings.models.providers["HydraROUTER"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `HydraROUTER/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove HydraROUTER settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove HydraROUTER from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["HydraROUTER"];
    ...
    // Remove HydraROUTER models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("HydraROUTER/"));
    ...
    // Reset agents.defaults.model.primary if it uses HydraROUTER
    if (settings.agents?.defaults?.model?.primary?.startsWith("HydraROUTER/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "HydraROUTER settings removed successfully",
    });
  } catch (error) {
