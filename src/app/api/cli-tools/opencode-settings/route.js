...
const hasHydraROUTERConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["HydraROUTER"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["HydraROUTER"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasHydraROUTER: hasHydraROUTERConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("HydraROUTER/") ? config.model.replace(/^HydraROUTER\//, "") : null,
...
// POST - Apply HydraROUTER as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_HydraROUTER";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing HydraROUTER provider entry and its models
    const existingProvider = config.provider["HydraROUTER"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["HydraROUTER"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `HydraROUTER/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `HydraROUTER/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("HydraROUTER/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove HydraROUTER provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["HydraROUTER"]?.models) {
      delete config.provider["HydraROUTER"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["HydraROUTER"].models).length === 0) {
        delete config.provider["HydraROUTER"];
        if (config.model?.startsWith("HydraROUTER/")) delete config.model;
      } else if (config.model === `HydraROUTER/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["HydraROUTER"].models);
        config.model = `HydraROUTER/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire HydraROUTER provider
      if (config.provider) delete config.provider["HydraROUTER"];
      if (config.model?.startsWith("HydraROUTER/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("HydraROUTER/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "HydraROUTER settings removed from OpenCode",
    });
  } catch (error) {
