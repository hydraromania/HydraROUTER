...
async function getInternalHeaders() {
  ...
  try {
    ...
    // Prefer unrestricted key (empty/null allowedModels), else any active key
    const unrestrictedKey = keys.find((k) => k.isActive !== false && (!k.allowedModels || (Array.isArray(k.allowedModels) && k.allowedModels.length === 0)));
    apiKey = unrestrictedKey?.key || keys.find((k) => k.isActive !== false)?.key || null;
  } catch {}
