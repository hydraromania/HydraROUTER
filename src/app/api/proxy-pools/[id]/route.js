...
function normalizeProxyPoolUpdate(body = {}) {
  ...
  // Auto-rotation state: 3x consecutive 429s set autoPausedUntil (+3h).
  // Accepts an ISO datetime or null (manual resume clears the pause).
  if (Object.prototype.hasOwnProperty.call(body, "autoPausedUntil")) {
    const raw = body?.autoPausedUntil;
    if (raw === null || raw === "") {
      updates.autoPausedUntil = null;
    } else {
      const t = new Date(raw).getTime();
      if (!Number.isFinite(t)) {
        return { error: "Invalid autoPausedUntil" };
      }
      updates.autoPausedUntil = new Date(t).toISOString();
    }
  }

  if (Object.prototype.hasOwnProperty.call(body, "autoConsecutive429")) {
    const n = Number(body?.autoConsecutive429);
    updates.autoConsecutive429 = Number.isFinite(n) && n >= 0 ? Math.floor(n) : 0;
  }
