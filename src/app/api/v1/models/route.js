...
import { filterModelsListForApiKey } from "@/lib/apiKeyModelFilter";
...
// and break recursive loops between HydraROUTER instances connected to each other.
...
export async function buildModelsList(kindFilter, options = {}) {
  ...
  // HydraROUTER instance's fetchCompatibleModelIds — skip dynamic fetch to break
  ...
  if (connections.length === 0) {
    ...
    for (const [providerId, conn] of activeConnectionByProvider.entries()) {
      ...
      if (kindFilter.includes("webFetch") && (providerInfo?.fetchConfig || providerInfo?.fetchViaChat)) {
        models.push({
...
export async function GET(request) {
  try {
    // Detect cross-instance recursive /models fetch (another HydraROUTER fetching our /models)
    const skipDynamicFetch = request?.headers?.get(INTERNAL_MODELS_FETCH_HEADER) === "1";
    let data = await buildModelsList([LLM_KIND], { skipDynamicFetch });

    // If API key is provided and has allowedModels configured, filter the list
    const apiKey = extractApiKey(request);
    data = await filterModelsListForApiKey(data, apiKey);
