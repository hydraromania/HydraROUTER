...
async function validateGeminiNativeClientKey(request, modelStr = null) {
  ...
  if (apiKey && modelStr) {
    ...
    if (!allowed) {
      return Response.json(
        { error: { message: `The model '${modelStr}' does not exist or you do not have access to it.` } },
        { status: 403 }
      );
    }
  }
...
async function forwardGeminiNativeRequest(request, body, model, action) {
  const modelId = normalizeGeminiNativeModel(model);
  const authError = await validateGeminiNativeClientKey(request, modelId);
