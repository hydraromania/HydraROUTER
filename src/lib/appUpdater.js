...
// Collect PIDs of all HydraROUTER-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from HydraROUTER install dir or wrapper cli.js
        const isAppProcess = lower.includes("HydraROUTER") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("HydraROUTER") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "HydraROUTER");
  }
  return path.join(os.homedir(), ".HydraROUTER");
}
...
// Resolve npx/HydraROUTER binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx HydraROUTER` — works regardless of global bin path changes after npm i -g
