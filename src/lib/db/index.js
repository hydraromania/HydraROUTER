...
export async function exportDb() {
  ...
  const out = {
    ...
    apiKeys: db.all(`SELECT * FROM apiKeys ORDER BY priority ASC, createdAt ASC`).map((r) => ({ id: r.id, key: r.key, name: r.name, machineId: r.machineId, isActive: r.isActive === 1, allowedModels: r.allowedModels ? parseJson(r.allowedModels, null) : null, outputFormat: r.outputFormat || "auto", priority: r.priority !== undefined ? Number(r.priority) : 0, isDefault: r.isDefault === 1 || r.isDefault === true, createdAt: r.createdAt })),
...
export async function importDb(payload) {
  ...
  db.transaction(() => {
    ...
    for (const k of payload.apiKeys || []) {
        db.run(
        `INSERT OR REPLACE INTO apiKeys(id, key, name, machineId, isActive, allowedModels, outputFormat, priority, isDefault, createdAt) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [k.id, k.key, k.name || null, k.machineId || null, k.isActive === false ? 0 : 1, k.allowedModels ? stringifyJson(k.allowedModels) : null, k.outputFormat || "auto", k.priority !== undefined ? Number(k.priority) : 0, k.isDefault ? 1 : 0, k.createdAt || new Date().toISOString()]
      );
    }
