...
function importLegacyMain(adapter, data) {
  ...
  importWithAssertion(adapter, "apiKeys", data.apiKeys || [], (k) => {
    adapter.run(
      ...
      [k.id, k.key, k.name || null, k.machineId || null, k.isActive === false ? 0 : 1, Array.isArray(k.allowedModels) ? stringifyJson(k.allowedModels) : null, k.outputFormat || "auto", k.priority !== undefined ? Number(k.priority) : 0, k.isDefault ? 1 : 0, k.createdAt || new Date().toISOString()]
    );
