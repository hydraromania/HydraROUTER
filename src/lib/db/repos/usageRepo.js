...
// Latest successful use timestamp (ms) for a provider/model pair, or 0.
// Backs idle-probe policies (e.g. nVidia) across process restarts.
export async function getLastModelUsageAt(provider, model) {
  try {
    const db = await getAdapter();
    const row = db.get(
      `SELECT timestamp FROM usageHistory WHERE provider = ? AND model = ? AND (status = 'ok' OR status = 'success') ORDER BY timestamp DESC LIMIT 1`,
      [provider, model]
    );
...
export async function getAnalyticsData({ period = "7d", provider = "all", model = "all", connectionId = "all" } = {}) {
  const db = await getAdapter();
  ...
  if (period === "today") {
    ...
    startOfDay.setHours(0, 0, 0, 0);
    const startTime = startOfDay.getTime();
    cutoffIso = new Date(startTime).toISOString();
    const labelFn = (ts) => new Date(ts).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: false });
    buckets = Array.from({ length: bucketCount }, (_, i) => ({
      key: String(i),
      label: labelFn(startTime + i * bucketMs),
      startTime: startTime + i * bucketMs,
      endTime: startTime + (i + 1) * bucketMs,
      requests: 0,
      promptTokens: 0,
      completionTokens: 0,
      cost: 0,
  ...
  // Fetch usage history rows matching cutoff
  let query = `SELECT timestamp, provider, model, connectionId, apiKey, promptTokens, completionTokens, cost, tokens, status FROM usageHistory WHERE timestamp >= ?`;
  ...
  if (connectionId && connectionId !== "all") {
    if (connectionId === "none" || connectionId === "no-account") {
      query += ` AND (connectionId IS NULL OR connectionId = '')`;
    } else {
  ...
  // Key / Connection comparisons
  const connectionStats = {};

  // Initialize connectionStats with all connections belonging to the selected provider (or all providers)
  for (const c of allConnections) {
    if (provider && provider !== "all" && c.provider !== provider) continue;
    connectionStats[c.id] = {
      connectionId: c.id,
      name: c.name || c.email || `Account ${c.id.slice(0, 8)}...`,
      email: c.email || null,
      provider: c.provider,
      providerDisplayName: providerNodeNameMap[c.provider] || c.provider,
  ...
  // Fallback bucket for requests without connectionId or unmapped connections
  const NO_CONN_ID = "__no_account__";
  connectionStats[NO_CONN_ID] = {
    connectionId: NO_CONN_ID,
    name: "Direct / No Account",
    email: null,
    provider: provider !== "all" ? provider : "multiple",
    providerDisplayName: provider !== "all" ? (providerNodeNameMap[provider] || provider) : "Direct API",
    authType: "apikey",
    isActive: true,
    priority: 999,
    requests: 0,
  ...
  for (const r of rows) {
    const rowProvider = r.provider || "unknown";
    const rowModel = r.model || "unknown";
    const rowConnId = r.connectionId || NO_CONN_ID;
    const rowCost = r.cost || 0;
    const isError = r.status && r.status !== "ok" && r.status !== "success";

    const tk = r.tokens ? parseJson(r.tokens, {}) : {};
    const promptTokens = r.promptTokens || tk.prompt_tokens || tk.input_tokens || 0;
    const completionTokens = r.completionTokens || tk.completion_tokens || tk.output_tokens || 0;

    distinctProviders.add(rowProvider);
    distinctModels.add(rowModel);
    if (r.connectionId) distinctConnections.add(r.connectionId);
    ...
    // Aggregate to connection stats
    if (!connectionStats[rowConnId]) {
      const connObj = connectionMap[rowConnId];
      connectionStats[rowConnId] = {
        connectionId: rowConnId,
        name: connObj?.name || connObj?.email || `Account ${rowConnId.slice(0, 8)}...`,
        email: connObj?.email || null,
        provider: rowProvider,
        providerDisplayName: providerNodeNameMap[rowProvider] || rowProvider,
        authType: connObj?.authType || "apikey",
        isActive: connObj ? connObj.isActive !== false : true,
        priority: connObj?.priority || 99,
        requests: 0,
        promptTokens: 0,
        completionTokens: 0,
        cost: 0,
    ...
    const cs = connectionStats[rowConnId];
    cs.requests += 1;
    cs.promptTokens += promptTokens;
    cs.completionTokens += completionTokens;
    cs.cost += rowCost;
    if (isError) cs.errors += 1;
    if (!cs.lastUsed || new Date(r.timestamp) > new Date(cs.lastUsed)) {
      cs.lastUsed = r.timestamp;
    }

    if (!cs.byModel[rowModel]) {
      cs.byModel[rowModel] = { requests: 0, promptTokens: 0, completionTokens: 0, cost: 0 };
    }
    cs.byModel[rowModel].requests += 1;
    cs.byModel[rowModel].promptTokens += promptTokens;
    cs.byModel[rowModel].completionTokens += completionTokens;
    cs.byModel[rowModel].cost += rowCost;

    // Aggregate to time buckets
    const t = new Date(r.timestamp).getTime();
    if (isHourly) {
      for (const b of buckets) {
        if (t >= b.startTime && t < b.endTime) {
          ...
          // Breakdown by model & connection in bucket
          const dimKey = rowModel;
          b.byDimension[dimKey] = (b.byDimension[dimKey] || 0) + 1;
          const connDimKey = `conn_${rowConnId}`;
          b.byDimension[connDimKey] = (b.byDimension[connDimKey] || 0) + 1;
          break;
        }
      }
    } else {
      ...
      const dateKey = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
      const b = buckets.find((bucket) => bucket.dateKey === dateKey);
      if (b) {
        b.requests += 1;
        b.promptTokens += promptTokens;
        b.completionTokens += completionTokens;
  ...
  // Clean up NO_CONN_ID if it has 0 requests and wasn't filtered
  if (connectionStats[NO_CONN_ID].requests === 0) {
    delete connectionStats[NO_CONN_ID];
  }

  // Transform connectionStats list with usage health / classification
  const connectionList = Object.values(connectionStats).map((c) => {
    return {
      ...c,
      isIdle: c.requests === 0,
      totalTokens: c.promptTokens + c.completionTokens,
      modelCount: Object.keys(c.byModel).length,
    };
  });

  // Sort connections by priority or active requests
  connectionList.sort((a, b) => {
    if (a.isIdle !== b.isIdle) return a.isIdle ? 1 : -1;
    return b.requests - a.requests;
  });

  const totalKeys = connectionList.length;
  const activeKeys = connectionList.filter((c) => c.requests > 0).length;
  const idleKeys = totalKeys - activeKeys;
  ...
  return {
    ...
    filters: {
      ...
      model,
      connectionId,
      availableProviders: Array.from(distinctProviders),
      availableModels: Array.from(distinctModels),
    },
    summary: {
      totalRequests,
      totalPromptTokens,
