...
const previousDefaultRegExp = /^# HydraROUTER-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# HydraROUTER-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via HydraROUTER gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# HydraROUTER-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# HydraROUTER-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
