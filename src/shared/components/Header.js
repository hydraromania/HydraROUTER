...
const getPageInfo = (pathname) => {
  ...
  if (pathname === "/dashboard" || pathname.includes("/usage"))
    return {
  ...
  if (pathname.includes("/endpoint"))
    return {
      title: "Endpoint & Keys",
      description: "API endpoint configuration and access keys",
      icon: "api",
      breadcrumbs: [],
    };
  ...
  if (pathname.includes("/mitm"))
    return {
      title: "MITM Proxy",
      description: "Intercept CLI tool traffic and route through HydraROUTER",
      icon: "security",
      breadcrumbs: [],
    };
  ...
  if (pathname.includes("/skills"))
    return {
      title: "Agent Skills",
      description: "Copy a link and paste to your AI to use HydraROUTER — no install needed",
      icon: "extension",
      breadcrumbs: [],
    };
  if (pathname.includes("/endpoint"))
...
export default function Header({ onMenuClick, showMenuButton = true }) {
  ...
  return (
    <header className="shrink-0 flex items-center justify-between gap-3 px-4 lg:px-8 py-3.5 border-b border-border-subtle glass-panel z-20 transition-all">
      {/* Mobile menu button */}
      <div className="flex items-center gap-3 lg:hidden shrink-0">
        {showMenuButton && (
          <button
            onClick={onMenuClick}
            className="p-1.5 rounded-lg text-text-main hover:bg-surface-2 transition-colors cursor-pointer"
          >
        ...
        {breadcrumbs.length > 0 ? (
                  ...
                  <div className="flex items-center gap-2">
                    {crumb.image && (
                      <ProviderIcon
                        ...
                        alt={crumb.label}
                        size={28}
                        className="object-contain rounded-lg max-w-[28px] max-h-[28px] shadow-sm"
          ...
          <div>
            <div className="flex items-center gap-2.5">
              {icon && (
                <div className="flex size-7 items-center justify-center rounded-lg bg-brand-500/10 text-brand-500 dark:text-brand-400">
                  <span className="material-symbols-outlined text-[18px]">
                    {icon}
                  </span>
                </div>
              )}
              <h1 className="text-base lg:text-xl font-bold text-text-main tracking-tight">
                {title}
              </h1>
            </div>
            {description && (
