...
export default function ModelSelectModal({
  ...
  closeOnSelect = true,
    ...
    // Include both LLM and Embedding connections when kindFilter is not set (so users can select embedding models in Combos)
    const filteredActiveProviders = useMemo(() => {
      if (!kindFilter) {
        return activeProviders.filter((p) => {
          const info = AI_PROVIDERS[p.provider];
          const kinds = info?.serviceKinds || ["llm"];
          return kinds.includes("llm") || kinds.includes("embedding");
        });
      }
      return activeProviders.filter((p) => {
        const info = AI_PROVIDERS[p.provider];
  ...
  const groupedModels = useMemo(() => {
    ...
    const PROVIDER_AS_MODEL_KINDS = new Set(["webFetch"]);
    ...
    const filterByKind = (models) => {
      ...
      if (!kindFilter) return models.filter((m) => m.isPlaceholder || m.isCustom || !getModelKind(m) || getModelKind(m) === "llm" || getModelKind(m) === "embedding");
      if (kindFilter === "webSearch") {
        return models.filter((m) => getModelKind(m) === "webSearch" || !getModelKind(m) || getModelKind(m) === "llm");
      }
    ...
    if (!kindFilter) {
      // Also show noAuth embedding providers when selecting models for combos
      NO_AUTH_PROVIDER_IDS.filter((id) => (AI_PROVIDERS[id]?.serviceKinds || ["llm"]).includes("embedding")).forEach(id => providerIdsToShow.add(id));
    }
    ...
    sortedProviderIds.forEach((providerId) => {
      ...
      if (providerInfo.passthroughModels) {
        ...
        if (kindFilter && TYPED_KINDS.has(kindFilter)) {
          ...
          const registeredLlms = customRegisteredModels.filter((m) => !getModelKind(m) || getModelKind(m) === "llm" || getModelKind(m) === "embedding");
          const seen = new Set([...aliasModels, ...registeredLlms].map((m) => m.value));
          const hardcoded = getModelsByProviderId(providerId)
            .filter((m) => !getModelKind(m) || getModelKind(m) === "llm" || getModelKind(m) === "embedding")
            .map((m) => ({ id: m.id, name: m.name, value: `${alias}/${m.id}`, kind: getModelKind(m) }))
        ...
        const customRegisteredModels = customModels
          ...
          .map((m) => ({ id: m.id, name: m.name || m.id, value: `${alias}/${m.id}`, isCustom: true, kind: getModelKind(m) }));
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {isPlaceholder ? (
                        ...
                        <>
                          {model.name}
                          {model.dimensions && (
                            <span className="text-[9px] opacity-80 font-mono bg-black/10 dark:bg-white/10 px-1 rounded ml-0.5">
                              {model.dimensions}d
                            </span>
                          )}
                          <CapacityBadges caps={getCaps(model.value)} />
