...
// Keep providers without serviceKinds (default LLM) or with "llm" or "embedding" in serviceKinds
...
function RecentRequests({ requests = [] }) {
  return (
    <div className="flex min-w-0 flex-col overflow-hidden rounded-2xl glass-panel shadow-sm" style={{ height: 480 }}>
      {/* Header */}
      <div className="flex items-center justify-between border-b border-border/60 bg-bg-subtle/40 px-4 py-3 shrink-0">
        <div className="flex items-center gap-2">
          <span className="flex size-2.5 rounded-full bg-emerald-500 animate-pulse glow-emerald" />
          <span className="text-xs font-bold text-text-muted uppercase tracking-wider">Live Stream Activity</span>
        </div>
        <span className="rounded-full bg-surface-2 px-2.5 py-0.5 text-[11px] font-semibold text-text-muted border border-border/40">
          {requests.length} stream logs
        </span>
      </div>

      {!requests.length ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-2 text-text-muted text-sm p-6 text-center">
          <span className="material-symbols-outlined text-[32px] opacity-40">hourglass_empty</span>
          <span>Waiting for real-time requests...</span>
        </div>
      ) : (
        <div className="flex-1 overflow-y-auto divide-y divide-border/40">
          {requests.map((r, i) => {
            ...
            return (
              <div
                key={i}
                className="group flex items-center justify-between gap-3 px-4 py-2.5 hover:bg-bg-subtle/60 transition-colors"
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  <span
                    className={`size-2 shrink-0 rounded-full transition-transform group-hover:scale-125 ${
                      ok ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]"
                    }`}
                  />
                  <div className="min-w-0">
                    <div className="truncate font-mono text-xs font-medium text-text-main group-hover:text-primary transition-colors" title={r.model}>
                      {r.model}
                    </div>
                    {r.provider && (
                ...
                <div className="flex items-end flex-col shrink-0">
                  <div className="flex items-center gap-1.5 font-mono text-xs font-semibold">
                    <span className="text-brand-500 bg-brand-500/10 px-1 rounded text-[11px]">{fmt(r.promptTokens)}↑</span>
                    <span className="text-emerald-500 bg-emerald-500/10 px-1 rounded text-[11px]">{fmt(r.completionTokens)}↓</span>
                  </div>
    ...
    <div className="flex min-w-0 flex-col gap-6">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            <div className="relative">
              <select
                ...
                className="w-full rounded-xl border border-border-subtle bg-surface/90 px-3.5 py-2 text-xs font-semibold text-text-main shadow-sm backdrop-blur-md focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 sm:w-auto"
          ...
          <div className="flex items-center rounded-xl border border-border/60 bg-bg-subtle/70 p-1 self-start sm:self-auto shadow-inner">
            <button
