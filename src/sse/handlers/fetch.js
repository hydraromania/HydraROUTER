...
async function handleSingleProviderFetch(body, providerInput, request, apiKey, settings) {
  ...
  const supportsFetch = !!providerConfig || !!resolvedProvider.fetchViaChat;
  if (!supportsFetch) {
  ...
  if (resolvedProvider.noAuth) {
    ...
    const result = await handleFetchCore({
      ...
      fallbackModels: resolvedProvider.fetchViaChat?.fallbackModels,
    });
    if (result.success) {
  ...
  const isChatFetch = !providerConfig && !!resolvedProvider.fetchViaChat;
  const primaryModel = requestedModel || resolvedProvider.fetchViaChat?.defaultModel || null;
  const modelChain = isChatFetch && primaryModel
    ? [primaryModel, ...((resolvedProvider.fetchViaChat?.fallbackModels || []).filter((m) => m && m !== primaryModel))]
    : [null];

  const fetchLockKeyFor = (chainModel) =>
    chainModel ? `webfetch:${providerId}/${chainModel}` : `webfetch:${providerId}`;
  ...
  while (true) {
    ...
    const fetchLockKey = fetchLockKeyFor(chainModel);
    const excludeForLookup = new Set([...excludeConnectionIds, ...spentForModel]);
    const credentials = await getProviderCredentials(providerId, excludeForLookup, fetchLockKey);

    if ((!credentials || credentials.allRateLimited) && isChatFetch && chainModel && chainIdx < modelChain.length - 1) {
      log.warn("FETCH", `[${providerId}/${chainModel}] all accounts exhausted, trying ${modelChain[chainIdx + 1]}`);
      chainIdx += 1;
    ...
    const result = await handleFetchCore({
      ...
      log,
      model: chainModel || requestedModel || body.model,
      fallbackModels: resolvedProvider.fetchViaChat?.fallbackModels,
