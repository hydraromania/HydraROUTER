...
function withConnectionHeader(response, connectionId) {
  ...
  headers.set("x-HydraROUTER-connection-id", String(connectionId));
...
export async function handleVideoCreate(request, action) {
  ...
  const apiKey = extractApiKey(request);
  if (apiKey && bodyInfo.parsed?.model) {
    const allowed = await isModelAllowedForApiKey(apiKey, String(bodyInfo.parsed.model));
    if (!allowed) {
      log.warn("AUTH", `API key does not have access to model '${bodyInfo.parsed.model}'`);
      return errorResponse(HTTP_STATUS.FORBIDDEN, `The model '${bodyInfo.parsed.model}' does not exist or you do not have access to it.`);
    }
  }
