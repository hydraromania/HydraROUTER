import { getProviderConnections, validateApiKey, updateProviderConnection, getSettings, getProxyPools, getApiKeyByKey, getCombos } from "@/lib/localDb";
...
export async function getProviderCredentials(provider, excludeConnectionIds = null, model = null, options = {}) {
  ...
  try {
    ...
    if (FREE_PROVIDERS[providerId]?.noAuth) {
      ...
      return {
        ...
        providerSpecificData: {
          ...
          proxyPoolAuto: resolvedProxy.isAuto === true,
          proxyPoolAutoExhausted: resolvedProxy.autoExhausted === true,
    ...
    // Filter out model-locked, excluded, rate-limited (per key & model), and Antigravity quota-exhausted connections.
    const availableConnections = [];
    for (const c of connections) {
      if (excludeSet.has(c.id)) continue;
      if (isModelLockActive(c, model)) continue;
      if (model && providerId) {
        if (await rateLimitTracker.isRateLimited(c.id, model, providerId)) {
          continue;
        }
        const usage = await rateLimitTracker.getUsage(c.id, model, providerId);
        const limits = getModelRateLimits(providerId, model);
        if (usage) {
    ...
    const resolvedProxy = await resolveConnectionProxyConfig(connection.providerSpecificData || {}, { rotationKey: connection.id });
    ...
    return {
      ...
      providerSpecificData: {
        ...
        connectionNoProxy: resolvedProxy.connectionNoProxy,
        connectionProxyPoolId: resolvedProxy.proxyPoolId || null,
        // Auto proxy-pool mode ("__auto__" binding): the concrete pool used
        // for this request + flag so handlers can record 429/success outcomes.
        proxyPoolAuto: resolvedProxy.isAuto === true,
        proxyPoolAutoExhausted: resolvedProxy.autoExhausted === true,
        vercelRelayUrl: resolvedProxy.vercelRelayUrl || "",
      },
...
export { isModelAllowedForApiKey } from "@/lib/apiKeyModelFilter.js";
