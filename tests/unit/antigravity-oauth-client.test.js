// Guards the deduped Antigravity OAuth client: env-only, no secret committed.
import { describe, it, expect } from "vitest";

describe("antigravity oauth client (deduped)", () => {
  it("shared source resolves from env, no secret committed", async () => {
    const { ANTIGRAVITY_OAUTH_CLIENT } = await import("../../open-sse/providers/shared.js");
    expect(ANTIGRAVITY_OAUTH_CLIENT).toEqual({
      clientId: process.env.HYDRAROUTER_ANTIGRAVITY_CLIENT_ID || "",
      clientSecret: process.env.HYDRAROUTER_ANTIGRAVITY_CLIENT_SECRET || "",
    });
  });

  it("registry transport keeps clientId/clientSecret fields", async () => {
  ...
  it("google client shared by gemini-cli", async () => {
    const { GOOGLE_OAUTH_CLIENT } = await import("../../open-sse/providers/shared.js");
    expect(GOOGLE_OAUTH_CLIENT).toEqual({
      clientId: process.env.HYDRAROUTER_GOOGLE_CLIENT_ID || "",
      clientSecret: process.env.HYDRAROUTER_GOOGLE_CLIENT_SECRET || "",
    });
  ...
  it("src oauth.js imports shared client + keeps full shape", async () => {
    ...
    expect(src).not.toContain("GOCSPX-"); // no Google OAuth secret hardcoded here
    expect(src).not.toContain("apps.googleusercontent.com"); // no Google client id hardcoded here
  });
});
