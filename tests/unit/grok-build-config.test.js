...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("HydraROUTER");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "HydraROUTER-general-purpose",
      explore: "HydraROUTER-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.HydraROUTER\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.HydraROUTER-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.HydraROUTER-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# HydraROUTER-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.HydraROUTER-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("HydraROUTER-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.HydraROUTER-");
    expect(reset).not.toContain("HydraROUTER-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("HydraROUTER-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("HydraROUTER-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("HydraROUTER-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("HydraROUTER-explore");
    expect(getGrokSubagentSlot("plan")).toBe("HydraROUTER-plan");
